        ...
        if (zend_hash_find (svc_info->ht_opParams,
            ...
            if (strcmp (function_type, "MIXED") == 0) {
                ...
                result_node =
                    ...
                    operation_name,
                    om_node, out_msg_ctx,
                    class_info, use_mtom, request_xop TSRMLS_CC);
            }
        }
    } else {
        result_node =
    ...
    zend_try {
        ...
        if (NULL != class_info
            ...
            if (Z_TYPE (retval) == IS_OBJECT
                ...
                if (zend_hash_find
                    (Z_OBJPROP (retval), "str", sizeof ("str"),
                        (void **) & msg_tmp) == SUCCESS
                    && Z_TYPE_PP (msg_tmp) == IS_STRING) {
                    ...
                    if (res_payload) {
                        res_om_node =
                            wsf_util_deserialize_buffer (env, res_payload);
                    }
                }
                if (zend_hash_find
                    (Z_OBJPROP (retval), "attachments",
                        sizeof ("attachments"),
                        (void **) & msg_tmp) == SUCCESS
                    && Z_TYPE_PP (msg_tmp) == IS_ARRAY) {
