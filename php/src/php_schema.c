...
static int schema_simpleType (
    sdlPtr sdl,
    xmlAttrPtr tns,
    xmlNodePtr simpleType,
    sdlTypePtr cur_type);
static int schema_complexType (
    ...
    xmlNodePtr compType,
    sdlTypePtr cur_type);
static int schema_list (
    sdlPtr sdl,
    xmlAttrPtr tns,
    xmlNodePtr listType,
    sdlTypePtr cur_type);
static int schema_union (
    sdlPtr sdl,
    xmlAttrPtr tns,
    xmlNodePtr unionType,
    sdlTypePtr cur_type);
static int schema_simpleContent (
    sdlPtr sdl,
    xmlAttrPtr tns,
    xmlNodePtr simpCompType,
    sdlTypePtr cur_type);
static int schema_restriction_simpleContent (
    sdlPtr sdl,
    xmlAttrPtr tns,
    xmlNodePtr restType,
    sdlTypePtr cur_type,
...
static int schema_restriction_complexContent (
    ...
    xmlNodePtr restType,
    sdlTypePtr cur_type);
static int schema_extension_simpleContent (
    sdlPtr sdl,
    xmlAttrPtr tns,
    xmlNodePtr extType,
    sdlTypePtr cur_type);
static int schema_extension_complexContent (
    sdlPtr sdl,
    xmlAttrPtr tns,
    xmlNodePtr extType,
    sdlTypePtr cur_type);
static int schema_sequence (
    sdlPtr sdl,
    xmlAttrPtr tns,
    xmlNodePtr seqType,
    sdlTypePtr cur_type,
    sdlContentModelPtr model);
static int schema_all (
    sdlPtr sdl,
    xmlAttrPtr tns,
    xmlNodePtr extType,
    sdlTypePtr cur_type,
    sdlContentModelPtr model);
static int schema_choice (
    sdlPtr sdl,
    xmlAttrPtr tns,
    xmlNodePtr choiceType,
    sdlTypePtr cur_type,
    sdlContentModelPtr model);
static int schema_group (
    sdlPtr sdl,
    xmlAttrPtr tns,
    xmlNodePtr groupType,
    sdlTypePtr cur_type,
    sdlContentModelPtr model);
static int schema_any (
    sdlPtr sdl,
    xmlAttrPtr tns,
    xmlNodePtr extType,
    sdlTypePtr cur_type,
...
static int schema_element (
    ...
    sdlTypePtr cur_type,
    sdlContentModelPtr model);
static int schema_attribute (
    sdlPtr sdl,
    xmlAttrPtr tns,
    xmlNodePtr attrType,
    sdlTypePtr cur_type,
    sdlCtx * ctx);
static int schema_attributeGroup (
    sdlPtr sdl,
    xmlAttrPtr tns,
    xmlNodePtr attrType,
    sdlTypePtr cur_type,
...
static int schema_restriction_var_int (
    xmlNodePtr val,
    sdlRestrictionIntPtr * valptr);

static int schema_restriction_var_char (
    xmlNodePtr val,
    sdlRestrictionCharPtr * valptr);

static void schema_type_fixup (
    sdlCtx * ctx,
    sdlTypePtr type);
    ...
    const xmlChar * type)
{
    ...
    smart_str_0 (&nscat);
    if (zend_hash_find (sdl->encoders, nscat.c, nscat.len + 1,
            (void **) &enc_ptr) == SUCCESS) {
        enc = *enc_ptr;
        if (enc->details.ns) {
    ...
    if (enc_ptr == NULL) {
        zend_hash_update (sdl->encoders, nscat.c, nscat.len + 1, &enc,
...
schema_load_file (
    ...
    xmlChar * location,
    xmlAttrPtr tns,
    int import TSRMLS_DC)
{
    if (location != NULL &&
        !zend_hash_exists (&ctx->docs, (char *) location,
            xmlStrlen (location) + 1)) {
        xmlDocPtr doc;
        xmlNodePtr schema;
        xmlAttrPtr new_tns;
        ...
        if (doc == NULL) {
            soap_error1 (E_ERROR,
                "Parsing Schema: can't import schema from '%s'", location);
        }
        ...
        if (import) {
            if (ns != NULL && (new_tns == NULL
                    || xmlStrcmp (ns->children->content,
                        new_tns->children->content) != 0)) {
                xmlFreeDoc (doc);
                soap_error2 (E_ERROR,
                    "Parsing Schema: can't import schema from '%s', unexpected 'targetNamespace'='%s'",
            ...
            if (new_tns == NULL) {
                ...
                && xmlStrcmp (tns->children->content,
                    new_tns->children->content) != 0) {
                xmlFreeDoc (doc);
                soap_error1 (E_ERROR,
                    "Parsing Schema: can't include schema from '%s', different 'targetNamespace'",
                    location);
            }
        }
        zend_hash_add (&ctx->docs, (char *) location,
            xmlStrlen (location) + 1, (void **) &doc, sizeof (xmlDocPtr),
            NULL);
    ...
    xmlNodePtr schema TSRMLS_DC)
{
    ...
    if (tns == NULL) {
        tns =
            xmlSetProp (schema, BAD_CAST ("targetNamespace"), BAD_CAST (""));
    ...
    while (trav != NULL) {
        if (node_is_equal (trav, "include")) {
            ...
            if (location == NULL) {
                ...
                if (base == NULL) {
                    uri =
                        xmlBuildURI (location->children->content,
                        trav->doc->URL);
                } else {
            ...
            if (ns != NULL && tns != NULL
                ...
                if (location) {
                    soap_error1 (E_ERROR,
                        "Parsing Schema: can't import schema from '%s', namespace must not match the enclosing schema 'targetNamespace'",
...
*/
    ...
    while (trav != NULL) {
        if (node_is_equal (trav, "simpleType")) {
            ...
            soap_error1 (E_ERROR, "Parsing Schema: unexpected <%s> in schema",
                trav->name);
        }
        trav = trav->next;
    }
    ...
    sdlTypePtr cur_type)
{
    ...
    if (cur_type != NULL) {
        ...
        zend_hash_next_index_insert (sdl->types, &newType,
            sizeof (sdlTypePtr), (void **) &ptr);
        ...
        zend_hash_next_index_insert (sdl->encoders, &cur_type->encode,
            sizeof (encodePtr), NULL);
        ...
        if (cur_type == NULL) {
            ...
            zend_hash_update (cur_type->elements, newType->name,
                strlen (newType->name) + 1, &newType, sizeof (sdlTypePtr),
                (void **) &ptr);
        }
        cur_type = (*ptr);
    ...
    if (trav != NULL) {
        if (node_is_equal (trav, "restriction")) {
            ...
            soap_error1 (E_ERROR,
                "Parsing Schema: unexpected <%s> in simpleType", trav->name);
        }
    } else {
        soap_error0 (E_ERROR,
    ...
    if (trav != NULL) {
        soap_error1 (E_ERROR, "Parsing Schema: unexpected <%s> in simpleType",
            trav->name);
    }
    ...
    sdlTypePtr cur_type)
{
    ...
    if (itemType != NULL) {
        ...
        if (nsptr != NULL) {
            ...
            newType->encode =
                get_create_encoder (sdl, newType, nsptr->href,
                BAD_CAST (type));
    ...
    if (trav != NULL) {
        soap_error1 (E_ERROR, "Parsing Schema: unexpected <%s> in list",
            trav->name);
    }
    return TRUE;
}
    ...
    sdlTypePtr cur_type)
{
    ...
    if (memberTypes != NULL) {
        ...
        while (start != NULL && *start != '\0') {
            ...
            if (nsptr != NULL) {
                ...
                if (cur_type->elements == NULL) {
                    cur_type->elements = emalloc (sizeof (HashTable));
                    zend_hash_init (cur_type->elements, 0, NULL, delete_type,
                        0);
                }
                zend_hash_next_index_insert (cur_type->elements, &newType,
                    sizeof (sdlTypePtr), (void **) &tmp);
    ...
    while (trav != NULL) {
        if (node_is_equal (trav, "simpleType")) {
            ...
            soap_error1 (E_ERROR, "Parsing Schema: unexpected <%s> in union",
                trav->name);
        }
        trav = trav->next;
    }
    ...
    sdlTypePtr cur_type)
{
    ...
    if (trav != NULL) {
        if (node_is_equal (trav, "restriction")) {
            ...
            soap_error1 (E_ERROR,
                "Parsing Schema: unexpected <%s> in simpleContent",
                trav->name);
        }
    } else {
        soap_error0 (E_ERROR,
    ...
    if (trav != NULL) {
        soap_error1 (E_ERROR,
            "Parsing Schema: unexpected <%s> in simpleContent", trav->name);
    }
...
*/
static int
schema_restriction_simpleContent (
    sdlPtr sdl,
    xmlAttrPtr tns,
    xmlNodePtr restType,
    sdlTypePtr cur_type,
    int simpleType)
{
    ...
    if (base != NULL) {
        ...
        if (nsptr != NULL) {
            cur_type->encode =
                get_create_encoder (sdl, cur_type, nsptr->href,
                BAD_CAST (type));
        }
        if (type) {
            efree (type);
    ...
    if (!simpleType) {
        while (trav != NULL) {
            if (node_is_equal (trav, "attribute")) {
                ...
                soap_error1 (E_ERROR,
                    "Parsing Schema: unexpected <%s> in restriction",
                    trav->name);
            }
            trav = trav->next;
        }
    ...
    if (trav != NULL) {
        soap_error1 (E_ERROR,
            "Parsing Schema: unexpected <%s> in restriction", trav->name);
    }
...
*/
static int
schema_restriction_complexContent (
    sdlPtr sdl,
    xmlAttrPtr tns,
    xmlNodePtr restType,
    sdlTypePtr cur_type)
    ...
    sdlRestrictionIntPtr * valptr)
{
    ...
    (*valptr)->fixed = FALSE;
    if (fixed != NULL) {
        if (!strncmp ((char *) fixed->children->content, "true",
                sizeof ("true"))
            || !strncmp ((char *) fixed->children->content, "1",
                sizeof ("1")))
            (*valptr)->fixed = TRUE;
    }
    ...
    sdlTypePtr cur_type)
{
    ...
    while (trav != NULL) {
        if (node_is_equal (trav, "attribute")) {
            ...
            soap_error1 (E_ERROR,
                "Parsing Schema: unexpected <%s> in extension", trav->name);
        }
        trav = trav->next;
    }
    if (trav != NULL) {
        soap_error1 (E_ERROR, "Parsing Schema: unexpected <%s> in extension",
            trav->name);
    }
    return TRUE;
}
    ...
    sdlContentModelPtr model)
{
    ...
    while (trav != NULL) {
        if (node_is_equal (trav, "element")) {
            ...
            soap_error1 (E_ERROR, "Parsing Schema: unexpected <%s> in all",
                trav->name);
        }
        trav = trav->next;
    }
    ...
    sdlContentModelPtr model)
{
    ...
    if (name) {
        ...
        if (cur_type == NULL) {
            ...
            if (zend_hash_add (sdl->groups, key.c, key.len + 1,
                    (void **) &newType, sizeof (sdlTypePtr),
                    NULL) != SUCCESS) {
                soap_error1 (E_ERROR,
                    "Parsing Schema: group '%s' already defined", key.c);
            }
    ...
    if (trav != NULL) {
        if (node_is_equal (trav, "choice")) {
            ...
            soap_error1 (E_ERROR, "Parsing Schema: unexpected <%s> in group",
                trav->name);
        }
    }
    if (trav != NULL) {
    ...
    sdlContentModelPtr model)
{
    ...
    while (trav != NULL) {
        if (node_is_equal (trav, "element")) {
            ...
            soap_error1 (E_ERROR, "Parsing Schema: unexpected <%s> in choice",
                trav->name);
        }
        trav = trav->next;
    }
    ...
    sdlContentModelPtr model)
{
    ...
    while (trav != NULL) {
        if (node_is_equal (trav, "element")) {
            ...
            soap_error1 (E_ERROR,
                "Parsing Schema: unexpected <%s> in sequence", trav->name);
        }
        trav = trav->next;
    }
    return TRUE;
    ...
    sdlTypePtr cur_type)
{
    ...
    if (trav != NULL) {
        if (node_is_equal (trav, "restriction")) {
            ...
            soap_error1 (E_ERROR,
                "Parsing Schema: unexpected <%s> in complexContent",
                trav->name);
        }
    } else {
        soap_error0 (E_ERROR,
    ...
    sdlTypePtr cur_type)
{
        ...
        if (node_is_equal (trav, "simpleContent")) {
            ...
            while (trav != NULL) {
                if (node_is_equal (trav, "attribute")) {
                    ...
                    soap_error1 (E_ERROR,
                        "Parsing Schema: unexpected <%s> in complexType",
                        trav->name);
                }
                trav = trav->next;
            }
    ...
    if (trav != NULL) {
        soap_error1 (E_ERROR,
            "Parsing Schema: unexpected <%s> in complexType", trav->name);
    }
    return TRUE;
}
    ...
    sdlContentModelPtr model)
{
    ...
    if (name) {
        ...
        smart_str_0 (&key);
        if (zend_hash_add (addHash, key.c, key.len + 1, &newType,
                sizeof (sdlTypePtr), NULL) != SUCCESS) {
            if (cur_type == NULL) {
                soap_error1 (E_ERROR,
                    "Parsing Schema: element '%s' already defined", key.c);
            } else {
                zend_hash_next_index_insert (addHash, &newType,
    ...
    if (cur_type->form == XSD_FORM_DEFAULT) {
        ...
        while (parent) {
            if (node_is_equal_ex (parent, "schema", SCHEMA_NAMESPACE)) {
                xmlAttrPtr def;
                def =
                    get_attribute (parent->properties, "elementFormDefault");
                if (def == NULL
    ...
    while (trav != NULL) {
        if (node_is_equal (trav, "unique")) {
            ...
            soap_error1 (E_ERROR,
                "Parsing Schema: unexpected <%s> in element", trav->name);
        }
        trav = trav->next;
    }
    ...
    sdlCtx * ctx)
{
    ...
    if (name) {
        ...
        if (cur_type == NULL) {
            ...
            if (cur_type->attributes == NULL) {
                ...
                zend_hash_init (cur_type->attributes, 0, NULL,
                    delete_attribute, 0);
            }
            addHash = cur_type->attributes;
        }

        if (zend_hash_add (addHash, key.c, key.len + 1, &newAttr,
                sizeof (sdlAttributePtr), NULL) != SUCCESS) {
            soap_error1 (E_ERROR,
                "Parsing Schema: attribute '%s' already defined", key.c);
        }
        smart_str_free (&key);
    } else {
        soap_error0 (E_ERROR,
    ...
    while (attr != NULL) {
        if (attr_is_equal_ex (attr, "default", SCHEMA_NAMESPACE)) {
            ...
            } else if (strncmp ((char *) attr->children->content,
                    "unqualified", sizeof ("unqualified")) == 0) {
            ...
            if (strncmp ((char *) nsPtr->href, SCHEMA_NAMESPACE,
                ...
                if (!newAttr->extraAttributes) {
                    ...
                    zend_hash_init (newAttr->extraAttributes, 0, NULL,
                ...
                smart_str_0 (&key2);
                zend_hash_add (newAttr->extraAttributes, key2.c, key2.len + 1,
                    &ext, sizeof (sdlExtraAttributePtr), NULL);
                smart_str_free (&key2);
            }
        }
        attr = attr->next;
    ...
    if (trav != NULL) {
        soap_error1 (E_ERROR, "Parsing Schema: unexpected <%s> in attribute",
            trav->name);
    }
    return TRUE;
}
    ...
    sdlCtx * ctx)
{
    ...
    if (name) {
        if (cur_type == NULL) {
            ...
            if (zend_hash_add (ctx->attributeGroups, key.c, key.len + 1,
                    &newType, sizeof (sdlTypePtr), NULL) != SUCCESS) {
                soap_error1 (E_ERROR,
                    "Parsing Schema: attributeGroup '%s' already defined",
                    key.c);
            }
            cur_type = newType;
            smart_str_free (&key);
            ...
            zend_hash_next_index_insert (cur_type->attributes, &newAttr,
                sizeof (sdlAttributePtr), NULL);
            cur_type = NULL;
        }
    } else {
        soap_error0 (E_ERROR,
    ...
    while (trav != NULL) {
        if (node_is_equal (trav, "attribute")) {
            ...
            soap_error1 (E_ERROR,
                "Parsing Schema: unexpected <%s> in attributeGroup",
                trav->name);
        }
        trav = trav->next;
    }
    ...
    sdlAttributePtr attr)
{
    ...
    if (attr->ref != NULL) {
        if (ctx->attributes != NULL) {
            if (zend_hash_find (ctx->attributes, attr->ref,
                    strlen (attr->ref) + 1, (void **) &tmp) == SUCCESS) {
                ...
                if ((*tmp)->extraAttributes != NULL) {
                    ...
                    zend_hash_init (attr->extraAttributes,
                        zend_hash_num_elements ((*tmp)->extraAttributes),
                        NULL, delete_extra_attribute, 0);
                    zend_hash_copy (attr->extraAttributes,
                        (*tmp)->extraAttributes, copy_extra_attribute, &node,
                        sizeof (xmlNodePtr));
                }
    ...
    HashTable * ht)
{
    ...
    if (attr->ref != NULL) {
        if (ctx->attributeGroups != NULL) {
            if (zend_hash_find (ctx->attributeGroups, attr->ref,
                    strlen (attr->ref) + 1, (void **) &tmp) == SUCCESS) {
                if ((*tmp)->attributes) {
                    zend_hash_internal_pointer_reset ((*tmp)->attributes);
                    while (zend_hash_get_current_data ((*tmp)->attributes,
                            (void **) &tmp_attr) == SUCCESS) {
                        if (zend_hash_get_current_key_type ((*tmp)->
                                attributes) == HASH_KEY_IS_STRING) {
                            char *key;
                            uint key_len;
                            sdlAttributePtr newAttr;
                            ...
                            if (newAttr->extraAttributes) {
                                ...
                                zend_hash_init (ht,
                                    zend_hash_num_elements (newAttr->
                                        extraAttributes), NULL,
                                    delete_extra_attribute, 0);
                                zend_hash_copy (ht, newAttr->extraAttributes,
                                    copy_extra_attribute, &node,
                                    sizeof (xmlNodePtr));
                                newAttr->extraAttributes = ht;
                            }

                            zend_hash_get_current_key_ex ((*tmp)->attributes,
                                &key, &key_len, NULL, 0, NULL);
                            zend_hash_add (ht, key, key_len, &newAttr,
                                sizeof (sdlAttributePtr), NULL);
                            ...
                            zend_hash_get_current_key ((*tmp)->attributes,
                                NULL, &index, 0);
    ...
    sdlContentModelPtr model)
{
    ...
    case XSD_CONTENT_GROUP_REF:{
            ...
            if (ctx->sdl->groups
                && zend_hash_find (ctx->sdl->groups, model->u.group_ref,
                    strlen (model->u.group_ref) + 1,
                    (void **) &tmp) == SUCCESS) {
    ...
    sdlTypePtr type)
{
    ...
    if (type->attributes) {
        ...
        while (zend_hash_get_current_data (type->attributes,
                (void **) &attr) == SUCCESS) {
            if (zend_hash_get_current_key_type (type->attributes) ==
                HASH_KEY_IS_STRING) {
    ...
    sdlCtx * ctx)
{
    ...
    if (ctx->attributes) {
        ...
        while (zend_hash_get_current_data (ctx->attributes,
                (void **) &attr) == SUCCESS) {
            schema_attribute_fixup (ctx, *attr);
    ...
    if (ctx->attributeGroups) {
        ...
        while (zend_hash_get_current_data (ctx->attributeGroups,
                (void **) &type) == SUCCESS) {
    ...
    void *data)
{
    ...
    if (type->restrictions) {
        delete_restriction_var_int_persistent (&type->restrictions->
            minExclusive);
        delete_restriction_var_int_persistent (&type->restrictions->
            minInclusive);
        delete_restriction_var_int_persistent (&type->restrictions->
        ...
        delete_restriction_var_int_persistent (&type->restrictions->
            maxLength);
        delete_restriction_var_char_persistent (&type->restrictions->
            whiteSpace);
...
void
delete_extra_attribute_persistent (
    void *attribute)
{
    sdlExtraAttributePtr attr = *((sdlExtraAttributePtr *) attribute);
...
void
delete_restriction_var_char_persistent (
    void *srvc)
{
    sdlRestrictionCharPtr ptr = *((sdlRestrictionCharPtr *) srvc);
    if (ptr) {
