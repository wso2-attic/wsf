...
encodePtr get_encoder_ex(sdlPtr sdl, const char *nscat, int len)
{
	...
	if (zend_hash_find(&WSF_GLOBAL(defEnc), (char*)nscat, len + 1, (void **)&enc) == SUCCESS) {
		return (*enc);
...
sdlPtr get_sdl(zval *this_ptr, char *uri, long cache_wsdl TSRMLS_DC)
{
	...
	if ((cache_wsdl & WSDL_CACHE_MEMORY) && WSF_GLOBAL(mem_cache)) {
		...
		if (SUCCESS == zend_hash_find(WSF_GLOBAL(mem_cache), uri, uri_len+1, (void*)&p)) {
	...
	if ((cache_wsdl & WSDL_CACHE_DISK) && (uri_len < MAXPATHLEN)) {
		...
		if ((sdl = get_sdl_from_cache(key, uri, t-WSF_GLOBAL(cache_ttl), &cached TSRMLS_CC)) != NULL) {
			t = cached;
			efree(key);
	...
	if (cache_wsdl & WSDL_CACHE_MEMORY) {
		if (sdl) {
			...
			if (WSF_GLOBAL(mem_cache) == NULL) {
				...
				zend_hash_init(WSF_GLOBAL(mem_cache), 0, NULL, delete_psdl, 1);
			} else if (WSF_GLOBAL(cache_limit) > 0 &&
			           WSF_GLOBAL(cache_limit) <= zend_hash_num_elements(WSF_GLOBAL(mem_cache))) {
				...
				for (zend_hash_internal_pointer_reset_ex(WSF_GLOBAL(mem_cache), &pos);
					 zend_hash_get_current_data_ex(WSF_GLOBAL(mem_cache), (void **) &q, &pos) == SUCCESS;
					 zend_hash_move_forward_ex(WSF_GLOBAL(mem_cache), &pos)) {
					if (q->time < latest) {
						latest = q->time;
						zend_hash_get_current_key_ex(WSF_GLOBAL(mem_cache), &key, &key_len, &idx, 0, &pos);
					}
				}
				if (key) {
