...
static void delete_fault (
    void *fault);
static void delete_fault_persistent (
    void *fault);
static void delete_binding (
    void *binding);
static void delete_binding_persistent (
    void *binding);
static void delete_function (
    void *function);
static void delete_function_persistent (
    void *function);
static void delete_parameter (
    void *paramater);
static void delete_parameter_persistent (
    void *paramater);
static void delete_header (
    void *header);
static void delete_header_persistent (
    void *header);
    ...
    const xmlChar * type)
{
    ...
    if (sdl->elements) {
        ...
        if (nsptr != NULL) {
            ...
            if (zend_hash_find (sdl->elements, nscat, len + 1,
                    (void **) &sdl_type) == SUCCESS) {
                ret = *sdl_type;
            } else if (zend_hash_find (sdl->elements, (char *) type,
                    type_len + 1, (void **) &sdl_type) == SUCCESS) {
                ret = *sdl_type;
            }
            efree (nscat);
        } else {
            if (zend_hash_find (sdl->elements, (char *) type,
                    xmlStrlen (type) + 1, (void **) &sdl_type) == SUCCESS) {
                ret = *sdl_type;
            }
        }
    ...
    const char *type)
{
    ...
    if (enc == NULL &&
        ...
        if (enc && sdl) {
            encodePtr new_enc =
                pemalloc (sizeof (encode), sdl->is_persistent);
            ...
            if (sdl->encoders == NULL) {
                sdl->encoders =
                    pemalloc (sizeof (HashTable), sdl->is_persistent);
                zend_hash_init (sdl->encoders, 0, NULL, delete_encoder,
                    sdl->is_persistent);
            }
            zend_hash_update (sdl->encoders, nscat, len + 1, &new_enc,
    ...
    int len)
{
    ...
    if (zend_hash_find (&WSF_GLOBAL (defEnc), (char *) nscat, len + 1,
            (void **) &enc) == SUCCESS) {
        return (*enc);
    } else if (sdl && sdl->encoders
        && zend_hash_find (sdl->encoders, (char *) nscat, len + 1,
            (void **) &enc) == SUCCESS) {
        return (*enc);
    }
    return NULL;
    ...
    int type)
{
    ...
    for (zend_hash_internal_pointer_reset (sdl->bindings);
        zend_hash_get_current_data (sdl->bindings,
            (void **) &binding) == SUCCESS;
    ...
    xmlNodePtr node)
{
    if (node->ns && strcmp ((char *) node->ns->href, WSDL_NAMESPACE) != 0) {
        xmlAttrPtr attr;
        if ((attr =
                get_attribute_ex (node->properties, "required",
                    WSDL_NAMESPACE)) != NULL && attr->children
            && attr->children->content
            && (strcmp ((char *) attr->children->content, "1") == 0
                || strcmp ((char *) attr->children->content, "true") == 0)) {
            soap_error1 (E_ERROR,
                "Parsing WSDL: Unknown required WSDL extension '%s'",
                node->ns->href);
        }
        return 0;
    }
    ...
    int include TSRMLS_DC)
{
    ...
    if (!definitions) {
        ...
        soap_error1 (E_ERROR,
            "Parsing WSDL: Couldn't find <definitions> in '%s'", struri);
    }

    if (!include) {
        targetNamespace =
            get_attribute (definitions->properties, "targetNamespace");
        if (targetNamespace) {
            tmpsdl->target_ns =
                estrdup ((char *) targetNamespace->children->content);
        }
    }
        ...
        if (node_is_equal (trav, "types")) {
            ...
            while (trav2 != NULL) {
                if (node_is_equal_ex (trav2, "schema", XSD_NAMESPACE)) {
                    ...
                    soap_error1 (E_ERROR,
                        "Parsing WSDL: Unexpected WSDL element <%s>",
                        trav2->name);
                }
                trav2 = trav2->next;
            }
            ...
            if (tmp) {
                ...
                if (base == NULL) {
                    uri =
                        xmlBuildURI (tmp->children->content, trav->doc->URL);
                } else {
            ...
            if (name && name->children && name->children->content) {
                if (zend_hash_add (&ctx->messages,
                        ...
                        xmlStrlen (name->children->content) + 1, &trav,
                        sizeof (xmlNodePtr), NULL) != SUCCESS) {
                    soap_error1 (E_ERROR,
                        "Parsing WSDL: <message> '%s' already defined",
                        name->children->content);
                }
            } else {
                soap_error0 (E_ERROR,
                    "Parsing WSDL: <message> hasn't name attribute");
            }

        } else if (node_is_equal (trav, "portType")) {
            xmlAttrPtr name = get_attribute (trav->properties, "name");
            if (name && name->children && name->children->content) {
                if (zend_hash_add (&ctx->portTypes,
                        (char *) name->children->content,
                        xmlStrlen (name->children->content) + 1, &trav,
                        sizeof (xmlNodePtr), NULL) != SUCCESS) {
                    soap_error1 (E_ERROR,
                        "Parsing WSDL: <portType> '%s' already defined",
                        name->children->content);
                }
            } else {
                soap_error0 (E_ERROR,
                    "Parsing WSDL: <portType> hasn't name attribute");
            }

        } else if (node_is_equal (trav, "binding")) {
            xmlAttrPtr name = get_attribute (trav->properties, "name");
            if (name && name->children && name->children->content) {
                if (zend_hash_add (&ctx->bindings,
                        (char *) name->children->content,
                        xmlStrlen (name->children->content) + 1, &trav,
                        sizeof (xmlNodePtr), NULL) != SUCCESS) {
                    soap_error1 (E_ERROR,
                        "Parsing WSDL: <binding> '%s' already defined",
                        name->children->content);
                }
            } else {
                soap_error0 (E_ERROR,
                    "Parsing WSDL: <binding> hasn't name attribute");
            }

        } else if (node_is_equal (trav, "service")) {
            xmlAttrPtr name = get_attribute (trav->properties, "name");
            if (name && name->children && name->children->content) {
                if (zend_hash_add (&ctx->services,
                        (char *) name->children->content,
                        xmlStrlen (name->children->content) + 1, &trav,
                        sizeof (xmlNodePtr), NULL) != SUCCESS) {
                    soap_error1 (E_ERROR,
                        "Parsing WSDL: <service> '%s' already defined",
                        name->children->content);
                }
            } else {
                soap_error0 (E_ERROR,
                    "Parsing WSDL: <service> hasn't name attribute");
            }
        } else if (!node_is_equal (trav, "documentation")) {
            soap_error1 (E_ERROR,
                "Parsing WSDL: Unexpected WSDL element <%s>", trav->name);
        }
        trav = trav->next;
    }
}

static sdlSoapBindingFunctionHeaderPtr
    ...
    int fault)
{
    ...
    if (zend_hash_find (&ctx->messages, ctype, strlen (ctype) + 1,
        ...
        soap_error1 (E_ERROR,
            "Parsing WSDL: Missing <message> with name '%s'",
    ...
    part =
        get_node_with_attribute_ex ((*message)->children, "part",
        WSDL_NAMESPACE, "name", (char *) tmp->children->content, NULL);
    if (!part) {
        soap_error1 (E_ERROR, "Parsing WSDL: Missing part '%s' in <message>",
            tmp->children->content);
    }
    ...
    if (tmp
        && !strncmp ((char *) tmp->children->content, "encoded",
            sizeof ("encoded"))) {
        h->use = SOAP_ENCODED;
    } else {
        h->use = SOAP_LITERAL;
    ...
    if (h->use == SOAP_ENCODED) {
        ...
        if (tmp) {
            if (strncmp ((char *) tmp->children->content,
                ...
                soap_error1 (E_ERROR,
                    "Parsing WSDL: Unknown encodingStyle '%s'",
                    tmp->children->content);
            }
        } else {
    ...
    if (tmp != NULL) {
        h->encode =
            get_encoder_from_prefix (ctx->sdl, part, tmp->children->content);
    } else {
        ...
        while (trav != NULL) {
            if (node_is_equal_ex (trav, "headerfault", wsdl_soap_namespace)) {
                sdlSoapBindingFunctionHeaderPtr hf =
                    wsdl_soap_binding_header (ctx, trav, wsdl_soap_namespace,
                    1);
                smart_str key = { 0 };

                if (h->headerfaults == NULL) {
                    h->headerfaults = emalloc (sizeof (HashTable));
                    zend_hash_init (h->headerfaults, 0, NULL, delete_header,
                        0);
                }
                ...
                if (zend_hash_add (h->headerfaults, key.c, key.len + 1,
                        (void **) &hf,
                        sizeof (sdlSoapBindingFunctionHeaderPtr),
                        NULL) != SUCCESS) {
        ...
        if (node_is_equal_ex (trav, "body", wsdl_soap_namespace)) {
            ...
            if (tmp) {
                ...
                while (*parts) {
                    ...
                    while (zend_hash_get_current_data_ex (params,
                            (void **) &param, &pos) != FAILURE) {
                        if ((*param)->paramName
                            ...
                            *x_param = **param;
                            ...
                            zend_hash_next_index_insert (&ht, &x_param,
                                sizeof (sdlParamPtr), NULL);
                            found = 1;
                            break;
                        }
            ...
            smart_str_0 (&key);
            if (zend_hash_add (binding->headers, key.c, key.len + 1,
                    (void **) &h, sizeof (sdlSoapBindingFunctionHeaderPtr),
                    NULL) != SUCCESS) {
    ...
    xmlChar * message_name)
{
    ...
    if (zend_hash_find (&ctx->messages, ctype, strlen (ctype) + 1,
            (void **) &tmp) != SUCCESS) {
        soap_error1 (E_ERROR,
            "Parsing WSDL: Missing <message> with name '%s'", message_name);
    }
    message = *tmp;
    ...
    while (trav != NULL) {
        ...
        if (trav->ns != NULL
            && strcmp ((char *) trav->ns->href, WSDL_NAMESPACE) != 0) {
            soap_error1 (E_ERROR,
                "Parsing WSDL: Unexpected extensibility element <%s>",
                trav->name);
        }
        ...
        if (name == NULL) {
            soap_error1 (E_ERROR,
                "Parsing WSDL: No name associated with <part> '%s'",
                message->name);
        }
        ...
        if (type != NULL) {
            param->encode =
                get_encoder_from_prefix (ctx->sdl, part,
                type->children->content);
        } else {
            element = get_attribute (part->properties, "element");
            if (element != NULL) {
                param->element =
                    get_element (ctx->sdl, part, element->children->content);
                if (param->element) {
        ...
        zend_hash_next_index_insert (parameters, &param, sizeof (sdlParamPtr),
            NULL);
            ...
            while (trav != NULL) {
                ...
                while (trav2 != NULL) {
                    if (node_is_equal (trav2, "address") && trav2->ns) {
                        ...
                        } else if (!strncmp ((char *) trav2->ns->href,
                ...
                tmpbinding->location =
                    estrdup ((char *) location->children->content);

                ctype =
                    strrchr ((char *) bindingAttr->children->content, ':');
                if (ctype == NULL) {
                    ctype = (char *) bindingAttr->children->content;
                } else {
                    ++ctype;
                }
                if (zend_hash_find (&ctx.bindings, ctype, strlen (ctype) + 1,
                        (void *) &tmp) != SUCCESS) {
                    soap_error1 (E_ERROR,
                        "Parsing WSDL: No <binding> element with name '%s'",
                        ctype);
                }
                binding = *tmp;
                ...
                if (tmpbinding->bindingType == BINDING_SOAP) {
                    ...
                    soapBindingNode =
                        get_node_ex (binding->children, "binding",
                        wsdl_soap_namespace);
                    if (soapBindingNode) {
                        tmp =
                            ...
                            "style");
                        if (tmp
                            && !strncmp ((char *) tmp->children->content,
                                "rpc", sizeof ("rpc"))) {
                        ...
                        if (tmp) {
                            if (strncmp ((char *) tmp->children->content,
                                ...
                                soap_error1 (E_ERROR,
                                    "Parsing WSDL: PHP-SOAP doesn't support transport '%s'",
                                    tmp->children->content);
                            }
                        }
                    }
                ...
                if (zend_hash_find (&ctx.portTypes, ctype, strlen (ctype) + 1,
                        (void **) &tmp) != SUCCESS) {
                    soap_error1 (E_ERROR,
                        "Parsing WSDL: Missing <portType> with name '%s'",
                        name->children->content);
                }
                portType = *tmp;
                ...
                while (trav2 != NULL) {
                    ...
                    if ((tmpbinding->bindingType == BINDING_SOAP &&
                            node_is_equal_ex (trav2, "binding",
                                wsdl_soap_namespace))
                    ...
                    while (trav3 != NULL) {
                        if (tmpbinding->bindingType == BINDING_SOAP &&
                            node_is_equal_ex (trav3, "operation",
                                wsdl_soap_namespace)) {
                    ...
                    portTypeOperation =
                        get_node_with_attribute_ex (portType->children,
                        "operation", WSDL_NAMESPACE, "name",
                        (char *) op_name->children->content, NULL);
                    if (portTypeOperation == NULL) {
                        soap_error1 (E_ERROR,
                            "Parsing WSDL: Missing <portType>/<operation> with name '%s'",
                    ...
                    function->functionName =
                        estrdup ((char *) op_name->children->content);
                    ...
                    if (tmpbinding->bindingType == BINDING_SOAP) {
                        ...
                        memset (soapFunctionBinding, 0,
                            sizeof (sdlSoapBindingFunction));
                        soapBinding =
                            (sdlSoapBindingPtr) tmpbinding->bindingAttributes;
                        soapFunctionBinding->style = soapBinding->style;

                        soapOperation =
                            get_node_ex (operation->children, "operation",
                            wsdl_soap_namespace);
                        if (soapOperation) {
                            tmp =
                                ...
                                "soapAction");
                            if (tmp) {
                                soapFunctionBinding->soapAction =
                                    estrdup ((char *) tmp->children->content);
                            }
                    ...
                    input =
                        get_node_ex (portTypeOperation->children, "input",
                        WSDL_NAMESPACE);
                    if (input != NULL) {
                        xmlAttrPtr message, name;

                        message =
                            get_attribute (input->properties, "message");
                        if (message == NULL) {
                            soap_error1 (E_ERROR,
                                "Parsing WSDL: Missing name for <input> of '%s'",
                                op_name->children->content);
                        }
...
*/
                        ...
                        if (tmpbinding->bindingType == BINDING_SOAP) {
                            input =
                                get_node_ex (operation->children, "input",
                                WSDL_NAMESPACE);
                            if (input != NULL) {
                                sdlSoapBindingFunctionPtr soapFunctionBinding
                                    = function->bindingAttributes;
                                wsdl_soap_binding_body (&ctx, input,
                                    wsdl_soap_namespace,
                    ...
                    output =
                        get_node_ex (portTypeOperation->children, "output",
                        WSDL_NAMESPACE);
                    if (output != NULL) {
                        xmlAttrPtr message, name;

                        message =
                            get_attribute (output->properties, "message");
                        if (message == NULL) {
                            soap_error1 (E_ERROR,
                                "Parsing WSDL: Missing name for <output> of '%s'",
                                op_name->children->content);
                        }
...
*/
                        {
                            ...
                            memcpy (function->responseName + len, "Response",
                                sizeof ("Response"));
                        }

                        if (tmpbinding->bindingType == BINDING_SOAP) {
                            output =
                                get_node_ex (operation->children, "output",
                                WSDL_NAMESPACE);
                            if (output != NULL) {
                                sdlSoapBindingFunctionPtr soapFunctionBinding
                                    = function->bindingAttributes;
                                wsdl_soap_binding_body (&ctx, output,
                                    wsdl_soap_namespace,
                    ...
                    while (fault != NULL) {
                        if (node_is_equal_ex (fault, "fault", WSDL_NAMESPACE)) {
                            ...
                            if (name == NULL) {
                                soap_error1 (E_ERROR,
                                    "Parsing WSDL: Missing name for <fault> of '%s'",
                                    op_name->children->content);
                            }
                            message =
                            ...
                            f->name =
                                estrdup ((char *) name->children->content);
                            f->details =
                                wsdl_message (&ctx,
                            ...
                            if (f->details == NULL
                                ...
                                soap_error1 (E_ERROR,
                                    "Parsing WSDL: The fault message '%s' must have a single part",
                                    message->children->content);
                            }

                            if (tmpbinding->bindingType == BINDING_SOAP) {
                                xmlNodePtr soap_fault =
                                    get_node_with_attribute_ex (operation->
                                    children, "fault", WSDL_NAMESPACE, "name",
                                    f->name, NULL);
                                if (soap_fault != NULL) {
                                    xmlNodePtr trav = soap_fault->children;
                                    while (trav != NULL) {
                                        if (node_is_equal_ex (trav, "fault",
                                                wsdl_soap_namespace)) {
                                            xmlAttrPtr tmp;
                                            sdlSoapBindingFunctionFaultPtr
                                                binding;

                                            binding = f->bindingAttributes =
                                                emalloc (sizeof
                                                (sdlSoapBindingFunctionFault));
                                            memset (f->bindingAttributes, 0,
                                                sizeof
                                                (sdlSoapBindingFunctionFault));
                            ...
                            if (function->faults == NULL) {
                                ...
                                zend_hash_init (function->faults, 0, NULL,
                                    delete_fault, 0);
                            }
                            if (zend_hash_add (function->faults, f->name,
                                    strlen (f->name) + 1, (void **) &f,
                                    sizeof (sdlFaultPtr), NULL) != SUCCESS) {
                                soap_error2 (E_ERROR,
                                    "Parsing WSDL: <fault> with name '%s' already defined in '%s'",
                                    f->name, op_name->children->content);
                            }
                        }
                        fault = fault->next;
                    }
...

                    {
                        ...
                        if (zend_hash_add (&ctx.sdl->functions,
                                php_strtolower (tmp, len), len + 1, &function,
                                sizeof (sdlFunctionPtr), NULL) != SUCCESS) {
                            zend_hash_next_index_insert (&ctx.sdl->functions,
                                &function, sizeof (sdlFunctionPtr), NULL);
                        }
                        efree (tmp);
                        if (function->requestName != NULL
                            && strcmp (function->requestName,
                                function->functionName) != 0) {
                            if (ctx.sdl->requests == NULL) {
                                ctx.sdl->requests =
                                    emalloc (sizeof (HashTable));
                                zend_hash_init (ctx.sdl->requests, 0, NULL,
                                    NULL, 0);
                            }
                            tmp = estrdup (function->requestName);
                            len = strlen (tmp);
                            zend_hash_add (ctx.sdl->requests,
                                php_strtolower (tmp, len), len + 1, &function,
                                sizeof (sdlFunctionPtr), NULL);
                            efree (tmp);
                        }
                    }
                    trav2 = trav2->next;
                ...
                if (!ctx.sdl->bindings) {
                    ...
                    zend_hash_init (ctx.sdl->bindings, 0, NULL,
                        delete_binding, 0);
                }

                zend_hash_add (ctx.sdl->bindings, tmpbinding->name,
                    strlen (tmpbinding->name), &tmpbinding,
                    sizeof (sdlBindingPtr), NULL);
                trav = trav->next;
            }
    ...
    char **in)
{
    ...
    if (i > 0) {
        ...
        zend_hash_init (attr->extraAttributes, i, NULL,
    ...
    char **in)
{
    ...
    case XSD_CONTENT_CHOICE:
        ...
        while (i > 0) {
            ...
            zend_hash_next_index_insert (model->u.content, &x,
    ...
    char **in)
{
    ...
    WSDL_CACHE_GET_1 (type->nillable, char,
        in);
    ...
    if (**in == 1) {
        ...
        type->restrictions->minExclusive =
            sdl_deserialize_resriction_int (in);
        ...
        if (i > 0) {
            ...
            zend_hash_init (type->restrictions->enumeration, i, NULL,
                delete_restriction_var_char, 0);
            while (i > 0) {
                sdlRestrictionCharPtr x =
                    sdl_deserialize_resriction_char (in);
...
sdl_deserialize_soap_body (
    sdlSoapBindingFunctionBodyPtr body,
    encodePtr * encoders,
    sdlTypePtr * types,
    char **in)
{
    ...
    if (i > 0) {
        ...
        while (i > 0) {
            ...
            if (tmp->use == SOAP_ENCODED) {
                WSDL_CACHE_GET_1 (tmp->encodingStyle, sdlRpcEncodingStyle,
                    in);
            } else {
    ...
    time_t * cached TSRMLS_DC)
{
    ...
    if (strncmp (in, "wsdl", 4) != 0 || in[4] != WSDL_CACHE_VERSION
        || in[5] != '\0') {
        unlink (fn);
        efree (buf);
        return NULL;
                ...
                for (j = 0; j < num_faults; j++) {
                    ...
                    if (*in != 0) {
                        sdlSoapBindingFunctionFaultPtr binding =
                            ...
                            emalloc (sizeof (sdlSoapBindingFunctionFault));
                        memset (binding, 0,
                            sizeof (sdlSoapBindingFunctionFault));
                        WSDL_CACHE_GET_1 (binding->use, sdlEncodingUse, &in);
                        if (binding->use == SOAP_ENCODED) {
                            WSDL_CACHE_GET_1 (binding->encodingStyle,
                                sdlRpcEncodingStyle, &in);
                        } else {
    ...
    smart_str * out)
{
    ...
    if (zend_hash_get_current_key_ex (ht, &key, &key_len, &index, 0,
    ...
    smart_str * out)
{
    if (enc) {
        int *encoder_num;
        if (zend_hash_find (tmp_encoders, (char *) &enc, sizeof (enc),
                (void **) &encoder_num) == SUCCESS) {
    ...
    smart_str * out)
{
    if (type) {
        int *type_num;
        if (zend_hash_find (tmp_types, (char *) &type, sizeof (type),
                (void **) &type_num) == SUCCESS) {
    ...
    smart_str * out)
{
    ...
    if (type->restrictions) {
        ...
        sdl_serialize_resriction_int (type->restrictions->fractionDigits,
            out);
        ...
        if (i > 0) {
            ...
            zend_hash_internal_pointer_reset (type->restrictions->
                enumeration);
            while (zend_hash_get_current_data (type->restrictions->
                    enumeration, (void **) &tmp) == SUCCESS) {
    ...
    if (i > 0) {
        ...
        while (zend_hash_get_current_data (type->elements,
                (void **) &tmp) == SUCCESS) {
            sdl_serialize_key (type->elements, out);
            sdl_serialize_type (*tmp, tmp_encoders, tmp_types, out);
            zend_hash_add (tmp_elements, (char *) tmp, sizeof (*tmp), &i,
                sizeof (int), NULL);
            i--;
    ...
    if (i > 0) {
        ...
        while (zend_hash_get_current_data (type->attributes,
                (void **) &tmp) == SUCCESS) {
    ...
    smart_str * out)
{
    ...
    if (i > 0) {
        ...
        while (zend_hash_get_current_data (body->headers,
                (void **) &tmp) == SUCCESS) {
            ...
            if (j > 0) {
                ...
                while (zend_hash_get_current_data ((*tmp)->headerfaults,
                        (void **) &tmp2) == SUCCESS) {
                    ...
                    sdl_serialize_encoder_ref ((*tmp2)->encode, tmp_encoders,
                        out);
    ...
    sdlPtr sdl TSRMLS_DC)
{
...
#else
    f = open (fn, O_CREAT | O_WRONLY | O_EXCL | O_BINARY,
        S_IREAD | S_IWRITE | S_IROTH | S_IWOTH | S_IRGRP | S_IWGRP);
#endif
    if (f < 0) {
        return;
    }
    ...
    if (i > 0) {
        ...
        while (zend_hash_get_current_data (sdl->groups,
                (void **) &tmp) == SUCCESS) {
            zend_hash_add (&tmp_types, (char *) tmp, sizeof (*tmp),
                (void **) &type_num, sizeof (type_num), NULL);
            ++type_num;
    ...
    if (i > 0) {
        ...
        while (zend_hash_get_current_data (sdl->types,
                (void **) &tmp) == SUCCESS) {
            zend_hash_add (&tmp_types, (char *) tmp, sizeof (*tmp),
                (void **) &type_num, sizeof (type_num), NULL);
            ++type_num;
    ...
    if (i > 0) {
        ...
        while (zend_hash_get_current_data (sdl->elements,
                (void **) &tmp) == SUCCESS) {
            zend_hash_add (&tmp_types, (char *) tmp, sizeof (*tmp),
                (void **) &type_num, sizeof (type_num), NULL);
            ++type_num;
    ...
    if (i > 0) {
        ...
        while (zend_hash_get_current_data (sdl->encoders,
                (void **) &tmp) == SUCCESS) {
            zend_hash_add (&tmp_encoders, (char *) tmp, sizeof (*tmp),
                (void **) &encoder_num, sizeof (encoder_num), NULL);
            ++encoder_num;
    ...
    enc = defaultEncoding;
    while (enc->details.type != END_KNOWN_TYPES) {
        zend_hash_add (&tmp_encoders, (char *) &enc, sizeof (encodePtr),
            (void **) &encoder_num, sizeof (encoder_num), NULL);
        enc++;
        ++encoder_num;
    }
    ...
    if (i > 0) {
        ...
        while (zend_hash_get_current_data (sdl->bindings,
                (void **) &tmp) == SUCCESS) {
            ...
            zend_hash_add (&tmp_bindings, (char *) tmp, sizeof (*tmp),
                (void **) &binding_num, sizeof (binding_num), NULL);
            binding_num++;
            zend_hash_move_forward (sdl->bindings);
        }
    }
    ...
    if (i > 0) {
        ...
        while (zend_hash_get_current_data (&sdl->functions,
            ...
            if ((*tmp)->binding == NULL ||
                zend_hash_find (&tmp_bindings, (char *) &(*tmp)->binding,
                    sizeof ((*tmp)->binding),
                    (void **) &binding_num) != SUCCESS) {
            }
            WSDL_CACHE_PUT_INT (*binding_num, out);
            if (*binding_num >= 0) {
                if ((*tmp)->binding->bindingType == BINDING_SOAP
                    && (*tmp)->bindingAttributes != NULL) {
                    ...
                    sdl_serialize_soap_body (&binding->input, &tmp_encoders,
                        &tmp_types, out);
                    sdl_serialize_soap_body (&binding->output, &tmp_encoders,
                        &tmp_types, out);
                } else {
                    WSDL_CACHE_PUT_1 (0, out);
                }
            ...
            if ((*tmp)->faults) {
                ...
                WSDL_CACHE_PUT_INT (zend_hash_num_elements ((*tmp)->faults),
                    out);
                ...
                while (zend_hash_get_current_data ((*tmp)->faults,
                    ...
                    if ((*tmp)->binding->bindingType == BINDING_SOAP
                        && (*fault)->bindingAttributes != NULL) {
                        sdlSoapBindingFunctionFaultPtr binding =
                            (sdlSoapBindingFunctionFaultPtr) (*fault)->
                            bindingAttributes;
    ...
    if (i > 0) {
        ...
        while (zend_hash_get_current_data (sdl->requests,
                (void **) &tmp) == SUCCESS) {
            if (zend_hash_find (&tmp_functions, (char *) tmp, sizeof (*tmp),
                    (void **) &function_num) != SUCCESS) {
            }
...
make_persistent_sdl_encoder_ref (
    encodePtr * enc,
    HashTable * ptr_map,
    HashTable * bp_encoders)
{
    encodePtr *tmp;

    /* do not process defaultEncoding's here */
    if ((*enc) >= defaultEncoding
        && (*enc) < defaultEncoding + numDefaultEncodings) {
        return;
    }
    ...
    HashTable * ptr_map)
{
    ...
    zend_hash_init (pheaders, zend_hash_num_elements (headers), NULL,
        delete_header_persistent, 1);
    ...
    while (zend_hash_get_current_data (headers, (void **) &tmp) == SUCCESS) {
        ...
        *pheader = **tmp;
        ...
        if (pheader->encode->details.sdl_type) {
            if (zend_hash_find (ptr_map, (char *) &pheader->encode,
                    sizeof (encodePtr), (void **) &penc) == FAILURE) {
                assert (0);
            }
            pheader->encode = *penc;
        }
        if (pheader->element) {
            if (zend_hash_find (ptr_map, (char *) &pheader->element,
                    sizeof (sdlTypePtr), (void **) &ptype) == FAILURE) {
                assert (0);
            }
        ...
        if (pheader->headerfaults) {
            pheader->headerfaults =
                make_persistent_sdl_function_headers (pheader->headerfaults,
                ptr_map);
        }

        if (zend_hash_get_current_key_ex (headers, &key, &key_len, &index, 0,
                NULL) == HASH_KEY_IS_STRING) {
            zend_hash_add (pheaders, key, key_len, (void *) &pheader,
                sizeof (sdlSoapBindingFunctionHeaderPtr), NULL);
        } else {
            zend_hash_next_index_insert (pheaders, (void *) &pheader,
                sizeof (sdlSoapBindingFunctionHeaderPtr), NULL);
        }
    ...
    HashTable * ptr_map)
{
    ...
    if (body->headers) {
        body->headers =
            make_persistent_sdl_function_headers (body->headers, ptr_map);
    }
}
    ...
    HashTable * ptr_map)
{
    ...
    zend_hash_init (pparams, zend_hash_num_elements (params), NULL,
        delete_parameter_persistent, 1);
    ...
    while (zend_hash_get_current_data (params, (void **) &tmp) == SUCCESS) {
        ...
        *pparam = **tmp;
        ...
        if (pparam->encode && pparam->encode->details.sdl_type) {
            if (zend_hash_find (ptr_map, (char *) &pparam->encode,
                    sizeof (encodePtr), (void **) &penc) == FAILURE) {
                assert (0);
            }
            pparam->encode = *penc;
        ...
        if (pparam->element) {
            if (zend_hash_find (ptr_map, (char *) &pparam->element,
                    sizeof (sdlTypePtr), (void **) &ptype) == FAILURE) {
                assert (0);
            }
        ...
        if (zend_hash_get_current_key_ex (params, &key, &key_len, &index, 0,
                NULL) == HASH_KEY_IS_STRING) {
            zend_hash_add (pparams, key, key_len, (void *) &pparam,
                sizeof (sdlParamPtr), NULL);
        } else {
            zend_hash_next_index_insert (pparams, (void *) &pparam,
                sizeof (sdlParamPtr), NULL);
        }
    ...
    HashTable * ptr_map)
{
    ...
    zend_hash_init (pfaults, zend_hash_num_elements (faults), NULL,
        delete_fault_persistent, 1);
    ...
    while (zend_hash_get_current_data (faults, (void **) &tmp) == SUCCESS) {
        ...
        *pfault = **tmp;
        ...
        if (pfault->details) {
            pfault->details =
                make_persistent_sdl_parameters (pfault->details, ptr_map);
        }

        if (func->binding->bindingType == BINDING_SOAP
            && pfault->bindingAttributes) {
            ...
            *soap_binding =
                *(sdlSoapBindingFunctionFaultPtr) pfault->bindingAttributes;
            if (soap_binding->ns) {
        ...
        if (zend_hash_get_current_key_ex (faults, &key, &key_len, &index, 0,
                NULL) == HASH_KEY_IS_STRING) {
            zend_hash_add (pfaults, key, key_len, (void *) &pfault,
                sizeof (sdlParamPtr), NULL);
        } else {
            zend_hash_next_index_insert (pfaults, (void *) &pfault,
                sizeof (sdlParamPtr), NULL);
        }
    ...
    HashTable * bp_encoders)
{
    ...
    *pattr = *attr;
    ...
    if (pattr->extraAttributes) {
        ...
        zend_hash_init (pattr->extraAttributes,
            zend_hash_num_elements (attr->extraAttributes), NULL,
            delete_extra_attribute_persistent, 1);
        ...
        while (zend_hash_get_current_data (attr->extraAttributes,
            ...
            if (zend_hash_get_current_key_ex (attr->extraAttributes, &key,
                    &key_len, &index, 0, NULL) == HASH_KEY_IS_STRING) {
                zend_hash_add (pattr->extraAttributes, key, key_len,
                    (void *) &pextra, sizeof (sdlExtraAttributePtr), NULL);
            }
    ...
    HashTable * bp_encoders)
{
    ...
    *pmodel = *model;
    ...
    case XSD_CONTENT_ELEMENT:
        if (pmodel->u.element) {
            make_persistent_sdl_type_ref (&pmodel->u.element, ptr_map,
                bp_types);
        }
        break;
    ...
    case XSD_CONTENT_CHOICE:
        ...
        zend_hash_init (pmodel->u.content,
            zend_hash_num_elements (model->u.content), NULL,
            delete_model_persistent, 1);

        zend_hash_internal_pointer_reset (model->u.content);
        while (zend_hash_get_current_data (model->u.content,
                (void **) &tmp) == SUCCESS) {
            pcontent =
                make_persistent_sdl_model (*tmp, ptr_map, bp_types,
                bp_encoders);
            zend_hash_next_index_insert (pmodel->u.content,
                (void *) &pcontent, sizeof (sdlContentModelPtr), NULL);
            zend_hash_move_forward (model->u.content);
        }
        break;
    ...
    case XSD_CONTENT_GROUP:
        if (pmodel->u.group) {
            make_persistent_sdl_type_ref (&pmodel->u.group, ptr_map,
                bp_types);
        }
        break;
    ...
    HashTable * bp_encoders)
{
    ...
    *ptype = *type;
    ...
    if (ptype->restrictions) {
        ...
        *ptype->restrictions = *type->restrictions;
        ...
        if (ptype->restrictions->minExclusive) {
            make_persistent_restriction_int (&ptype->restrictions->
                minExclusive);
        }
        ...
        if (ptype->restrictions->whiteSpace) {
            make_persistent_restriction_char (&ptype->restrictions->
                whiteSpace);
        }
        ...
        if (type->restrictions->enumeration) {
            ...
            zend_hash_init (ptype->restrictions->enumeration,
                zend_hash_num_elements (type->restrictions->enumeration),
                NULL, delete_restriction_var_char_persistent, 1);
            zend_hash_copy (ptype->restrictions->enumeration,
                type->restrictions->enumeration,
                make_persistent_restriction_char, (void *) &tmp,
                sizeof (sdlRestrictionCharPtr));
        }
    }
    ...
    if (ptype->elements) {
        ...
        zend_hash_init (ptype->elements,
            zend_hash_num_elements (type->elements), NULL,
            delete_type_persistent, 1);

        zend_hash_internal_pointer_reset (type->elements);
        while (zend_hash_get_current_data (type->elements,
                (void **) &tmp) == SUCCESS) {
            pelem =
                make_persistent_sdl_type (*tmp, ptr_map, bp_types,
                bp_encoders);
            if (zend_hash_get_current_key_ex (type->elements, &key, &key_len,
                    &index, 0, NULL) == HASH_KEY_IS_STRING) {
                zend_hash_add (ptype->elements, key, key_len, (void *) &pelem,
                    sizeof (sdlTypePtr), NULL);
            } else {
                zend_hash_next_index_insert (ptype->elements, (void *) &pelem,
                    sizeof (sdlTypePtr), NULL);
            }
            zend_hash_add (ptr_map, (char *) tmp, sizeof (*tmp),
                (void *) &pelem, sizeof (sdlTypePtr), NULL);
            zend_hash_move_forward (type->elements);
        }
    }
    ...
    if (ptype->attributes) {
        ...
        zend_hash_init (ptype->attributes,
            zend_hash_num_elements (type->attributes), NULL,
            delete_attribute_persistent, 1);

        zend_hash_internal_pointer_reset (type->attributes);
        while (zend_hash_get_current_data (type->attributes,
                (void **) &tmp) == SUCCESS) {
            pattr =
                make_persistent_sdl_attribute (*tmp, ptr_map, bp_types,
                bp_encoders);
            if (zend_hash_get_current_key_ex (type->attributes, &key,
                    &key_len, &index, 0, NULL) == HASH_KEY_IS_STRING) {
                zend_hash_add (ptype->attributes, key, key_len,
                    (void *) &pattr, sizeof (sdlAttributePtr), NULL);
            } else {
                zend_hash_next_index_insert (ptype->attributes,
                    (void *) &pattr, sizeof (sdlAttributePtr), NULL);
            }
            zend_hash_move_forward (type->attributes);
        }
    ...
    if (type->model) {
        ptype->model =
            make_persistent_sdl_model (ptype->model, ptr_map, bp_types,
            bp_encoders);
    }
    ...
    HashTable * bp_encoders)
{
    ...
    *penc = *enc;
    ...
    if (penc->details.sdl_type) {
        make_persistent_sdl_type_ref (&penc->details.sdl_type, ptr_map,
            bp_types);
    }
    ...
    HashTable * ptr_map)
{
    ...
    *pfunc = *func;
    ...
    if (pfunc->binding) {
        ...
        if (zend_hash_find (ptr_map, (char *) &pfunc->binding,
                sizeof (pfunc->binding), (void **) &tmp) == FAILURE) {
            assert (0);
        }
        pfunc->binding = *tmp;

        if (pfunc->binding->bindingType == BINDING_SOAP
            && pfunc->bindingAttributes) {
            ...
            *soap_binding =
                *(sdlSoapBindingFunctionPtr) pfunc->bindingAttributes;
        ...
        if (pfunc->requestParameters) {
            pfunc->requestParameters =
                make_persistent_sdl_parameters (pfunc->requestParameters,
                ptr_map);
        }
        if (pfunc->responseParameters) {
            pfunc->responseParameters =
                make_persistent_sdl_parameters (pfunc->responseParameters,
                ptr_map);
        }
        if (pfunc->faults) {
            pfunc->faults =
                make_persistent_sdl_function_faults (pfunc, pfunc->faults,
                ptr_map);
        }
    }
    ...
    sdlPtr sdl TSRMLS_DC)
{
    ...
    if (sdl->groups) {
        ...
        zend_hash_init (psdl->groups, zend_hash_num_elements (sdl->groups),
            NULL, delete_type_persistent, 1);
        ...
        while (zend_hash_get_current_data (sdl->groups,
            ...
            ptype =
                make_persistent_sdl_type (*tmp, &ptr_map, &bp_types,
                &bp_encoders);
            if (zend_hash_get_current_key_ex (sdl->groups, &key, &key_len,
                    &index, 0, NULL) == HASH_KEY_IS_STRING) {
                zend_hash_add (psdl->groups, key, key_len, (void *) &ptype,
                    sizeof (sdlTypePtr), NULL);
            } else {
                zend_hash_next_index_insert (psdl->groups, (void *) &ptype,
                    sizeof (sdlTypePtr), NULL);
            }
            zend_hash_add (&ptr_map, (char *) tmp, sizeof (*tmp),
                (void *) &ptype, sizeof (sdlTypePtr), NULL);
            zend_hash_move_forward (sdl->groups);
        }
    }
    ...
    if (sdl->types) {
        ...
        zend_hash_init (psdl->types, zend_hash_num_elements (sdl->types),
            NULL, delete_type_persistent, 1);
        ...
        while (zend_hash_get_current_data (sdl->types,
            ...
            ptype =
                make_persistent_sdl_type (*tmp, &ptr_map, &bp_types,
                &bp_encoders);
            if (zend_hash_get_current_key_ex (sdl->types, &key, &key_len,
                    &index, 0, NULL) == HASH_KEY_IS_STRING) {
                zend_hash_add (psdl->types, key, key_len, (void *) &ptype,
                    sizeof (sdlTypePtr), NULL);
            } else {
                zend_hash_next_index_insert (psdl->types, (void *) &ptype,
                    sizeof (sdlTypePtr), NULL);
            }
            zend_hash_add (&ptr_map, (char *) tmp, sizeof (*tmp),
                (void *) &ptype, sizeof (sdlTypePtr), NULL);
    ...
    if (sdl->elements) {
        ...
        zend_hash_init (psdl->elements,
            zend_hash_num_elements (sdl->elements), NULL,
            delete_type_persistent, 1);
        ...
        while (zend_hash_get_current_data (sdl->elements,
            ...
            ptype =
                make_persistent_sdl_type (*tmp, &ptr_map, &bp_types,
                &bp_encoders);
            if (zend_hash_get_current_key_ex (sdl->elements, &key, &key_len,
                    &index, 0, NULL) == HASH_KEY_IS_STRING) {
                zend_hash_add (psdl->elements, key, key_len, (void *) &ptype,
                    sizeof (sdlTypePtr), NULL);
            } else {
                zend_hash_next_index_insert (psdl->elements, (void *) &ptype,
                    sizeof (sdlTypePtr), NULL);
            }
            zend_hash_add (&ptr_map, (char *) tmp, sizeof (*tmp),
                (void *) &ptype, sizeof (sdlTypePtr), NULL);
    ...
    if (sdl->encoders) {
        ...
        zend_hash_init (psdl->encoders,
            zend_hash_num_elements (sdl->encoders), NULL,
            delete_encoder_persistent, 1);

        zend_hash_internal_pointer_reset (sdl->encoders);
        while (zend_hash_get_current_data (sdl->encoders,
                (void **) &tmp) == SUCCESS) {
            penc =
                make_persistent_sdl_encoder (*tmp, &ptr_map, &bp_types,
                &bp_encoders);
            if (zend_hash_get_current_key_ex (sdl->encoders, &key, &key_len,
                    &index, 0, NULL) == HASH_KEY_IS_STRING) {
                zend_hash_add (psdl->encoders, key, key_len, (void *) &penc,
                    sizeof (encodePtr), NULL);
            } else {
                zend_hash_next_index_insert (psdl->encoders, (void *) &penc,
                    sizeof (encodePtr), NULL);
            }
            zend_hash_add (&ptr_map, (char *) tmp, sizeof (*tmp),
                (void *) &penc, sizeof (encodePtr), NULL);
            zend_hash_move_forward (sdl->encoders);
        }
    }
    ...
    if (zend_hash_num_elements (&bp_types)) {
        ...
        while (zend_hash_get_current_data (&bp_types,
                (void **) &tmp) == SUCCESS) {
            if (zend_hash_find (&ptr_map, (char *) (*tmp), sizeof (**tmp),
                    (void **) &ptype) == FAILURE) {
                assert (0);
            }
            **tmp = *ptype;
    ...
    if (zend_hash_num_elements (&bp_encoders)) {
        ...
        while (zend_hash_get_current_data (&bp_encoders,
                (void **) &tmp) == SUCCESS) {
            if (zend_hash_find (&ptr_map, (char *) (*tmp), sizeof (**tmp),
                    (void **) &penc) == FAILURE) {
                assert (0);
    ...
    if (sdl->bindings) {
        ...
        zend_hash_init (psdl->bindings,
            zend_hash_num_elements (sdl->bindings), NULL,
            delete_binding_persistent, 1);

        zend_hash_internal_pointer_reset (sdl->bindings);
        while (zend_hash_get_current_data (sdl->bindings,
                (void **) &tmp) == SUCCESS) {
            pbind = make_persistent_sdl_binding (*tmp, &ptr_map);
            if (zend_hash_get_current_key_ex (sdl->bindings, &key, &key_len,
                    &index, 0, NULL) == HASH_KEY_IS_STRING) {
                zend_hash_add (psdl->bindings, key, key_len, (void *) &pbind,
                    sizeof (sdlBindingPtr), NULL);
            } else {
                zend_hash_next_index_insert (psdl->bindings, (void *) &pbind,
                    sizeof (sdlBindingPtr), NULL);
            }
            zend_hash_add (&ptr_map, (char *) tmp, sizeof (*tmp),
                (void *) &pbind, sizeof (sdlBindingPtr), NULL);
            zend_hash_move_forward (sdl->bindings);
        }
    }

    zend_hash_init (&psdl->functions,
        zend_hash_num_elements (&sdl->functions), NULL,
        delete_function_persistent, 1);
    if (zend_hash_num_elements (&sdl->functions)) {
        ...
        while (zend_hash_get_current_data (&sdl->functions,
            ...
            if (zend_hash_get_current_key_ex (&sdl->functions, &key, &key_len,
                    &index, 0, NULL) == HASH_KEY_IS_STRING) {
                zend_hash_add (&psdl->functions, key, key_len,
                    (void *) &pfunc, sizeof (sdlFunctionPtr), NULL);
            } else {
                zend_hash_next_index_insert (&psdl->functions,
                    (void *) &pfunc, sizeof (sdlFunctionPtr), NULL);
            }
            zend_hash_add (&ptr_map, (char *) tmp, sizeof (*tmp),
                (void *) &pfunc, sizeof (sdlFunctionPtr), NULL);
    ...
    if (sdl->requests) {
        ...
        zend_hash_init (psdl->requests,
            zend_hash_num_elements (sdl->requests), NULL, NULL, 1);
        ...
        while (zend_hash_get_current_data (sdl->requests,
            ...
            *tmp = *preq;
            if (zend_hash_get_current_key_ex (sdl->requests, &key, &key_len,
                    &index, 0, NULL) == HASH_KEY_IS_STRING) {
                zend_hash_add (psdl->requests, key, key_len, (void *) &preq,
                    sizeof (sdlFunctionPtr), NULL);
            }
            zend_hash_move_forward (sdl->requests);
        }
    ...
    long cache_wsdl TSRMLS_DC)
{
    char fn[MAXPATHLEN];
    sdlPtr sdl = NULL;
    char *old_error_code = WSF_GLOBAL (error_code);
    int uri_len = 0;
    php_stream_context *context = NULL;
    zval **tmp, **proxy_host, **proxy_port, *orig_context =
        NULL, *new_context = NULL;
    smart_str headers = { 0 };
    char *key = NULL;
    time_t t = time (0);
    ...
    if ((cache_wsdl & WSDL_CACHE_MEMORY) && WSF_GLOBAL (mem_cache)) {
        sdl_cache_bucket *p;

        if (SUCCESS == zend_hash_find (WSF_GLOBAL (mem_cache), uri,
                uri_len + 1, (void *) &p)) {
            if (p->time < t - WSF_GLOBAL (cache_ttl)) {
    ...
    if ((cache_wsdl & WSDL_CACHE_DISK) && (uri_len < MAXPATHLEN)) {
        ...
        int len = strlen (WSF_GLOBAL (cache_dir));
        time_t cached;
        ...
        memcpy (key, WSF_GLOBAL (cache_dir), len);
        memcpy (key + len, "/wsdl-", sizeof ("/wsdl-") - 1);
        memcpy (key + len + sizeof ("/wsdl-") - 1, md5str, sizeof (md5str));

        if ((sdl =
                get_sdl_from_cache (key, uri, t - WSF_GLOBAL (cache_ttl),
                    &cached TSRMLS_CC)) != NULL) {
            t = cached;
            efree (key);
    ...
    if (zend_hash_find (Z_OBJPROP_P (this_ptr), "_proxy_host",
        ...
        php_stream_context_set_option (context, "http", "request_fulluri",
            str_proxy);
        zval_ptr_dtor (&str_proxy);
    ...
    WSF_GLOBAL (error_code) = old_error_code;
  ...
  cache_in_memory:
    if (cache_wsdl & WSDL_CACHE_MEMORY) {
        if (sdl) {
            ...
            if (WSF_GLOBAL (mem_cache) == NULL) {
                WSF_GLOBAL (mem_cache) = malloc (sizeof (HashTable));
                zend_hash_init (WSF_GLOBAL (mem_cache), 0, NULL, delete_psdl,
                    1);
            } else if (WSF_GLOBAL (cache_limit) > 0
                && WSF_GLOBAL (cache_limit) <=
                zend_hash_num_elements (WSF_GLOBAL (mem_cache))) {
                ...
                for (zend_hash_internal_pointer_reset_ex (WSF_GLOBAL
                        (mem_cache), &pos);
                    zend_hash_get_current_data_ex (WSF_GLOBAL (mem_cache),
                        (void **) &q, &pos) == SUCCESS;
                    zend_hash_move_forward_ex (WSF_GLOBAL (mem_cache),
                        &pos)) {
                    if (q->time < latest) {
                        latest = q->time;
                        zend_hash_get_current_key_ex (WSF_GLOBAL (mem_cache),
                            &key, &key_len, &idx, 0, &pos);
                    }
                }
                if (key) {
                    zend_hash_del (WSF_GLOBAL (mem_cache), key, key_len);
                } else {
                    return sdl;
                }
            }
            ...
            if (SUCCESS == zend_hash_update (WSF_GLOBAL (mem_cache), uri,
                    uri_len + 1, (void *) &p, sizeof (sdl_cache_bucket),
                    NULL)) {
...
delete_sdl_soap_binding_function_body (
    sdlSoapBindingFunctionBody body)
{
    if (body.ns) {
        efree (body.ns);
    }
    ...
    void *data)
{
    ...
    if (function->bindingAttributes &&
        ...
        delete_sdl_soap_binding_function_body_persistent (soapFunction->
            input);
        delete_sdl_soap_binding_function_body_persistent (soapFunction->
            output);
        free (soapFunction);
    ...
    void *data)
{
    sdlSoapBindingFunctionHeaderPtr hdr =
        *((sdlSoapBindingFunctionHeaderPtr *) data);
    if (hdr->name) {
        efree (hdr->name);
    }
    if (hdr->ns) {
