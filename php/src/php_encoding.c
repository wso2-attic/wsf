			...
			for (;i > 0;i--) {
				...
				if (key_type == HASH_KEY_IS_STRING) {
			  	char *prop_name;
					...
					if (Z_TYPE_P(data) == IS_OBJECT) {
						char *class_name;
#ifdef (PHP_MAJOR_VERSION == 5 && PHP_MINOR_VERSION < 2)
	...
	if (data == NULL) {
		...
		if (tmpattr != NULL) {
		  type_name = tmpattr->children->content;
			enc = get_encoder_from_prefix(WSF_GLOBAL(sdl), data, tmpattr->children->content);
...
xmlNsPtr encode_add_ns(xmlNodePtr node, const char* ns)
{
	...
	if (xmlns == NULL) {
		...
		if (zend_hash_find(&WSF_GLOBAL(defEncNs), (char*)ns, strlen(ns) + 1, (void **)&prefix) == SUCCESS) {
...
encodePtr get_conversion(int encode)
{
	...
	if (zend_hash_index_find(&WSF_GLOBAL(defEncIndex), encode, (void **)&enc) == FAILURE) {
...
static encodePtr get_array_type(xmlNodePtr node, zval *array, smart_str *type TSRMLS_DC)
{
	...
	if (different || count == 0) {
		...
		if (cur_stype != NULL) {
			...
			enc = get_encoder_ex(WSF_GLOBAL(sdl), array_type.c, array_type.len);
			smart_str_free(&array_type);
			return enc;
		} else {
