...
/* zval type decode */
static zval *to_zval_double (
    encodeTypePtr type,
    xmlNodePtr data);
static zval *to_zval_long (
    encodeTypePtr type,
    xmlNodePtr data);
static zval *to_zval_bool (
    encodeTypePtr type,
    xmlNodePtr data);
static zval *to_zval_string (
    encodeTypePtr type,
    xmlNodePtr data);
static zval *to_zval_stringr (
    encodeTypePtr type,
    xmlNodePtr data);
static zval *to_zval_stringc (
    encodeTypePtr type,
    xmlNodePtr data);
static zval *to_zval_map (
    encodeTypePtr type,
    xmlNodePtr data);
static zval *to_zval_null (
    encodeTypePtr type,
    xmlNodePtr data);
static zval *to_zval_base64 (
    encodeTypePtr type,
    xmlNodePtr data);
static zval *to_zval_hexbin (
    encodeTypePtr type,
    xmlNodePtr data);
...
static zval *guess_zval_convert (
    encodeTypePtr type,
    xmlNodePtr data);
...
static encodePtr get_array_type (
    xmlNodePtr node,
    zval * array,
    smart_str * out_type TSRMLS_DC);

static xmlNodePtr check_and_resolve_href (
    xmlNodePtr data);

static void set_ns_prop (
    xmlNodePtr node,
    char *ns,
    char *name,
    char *val);
static void set_xsi_nil (
    xmlNodePtr node);
static void set_xsi_type (
    xmlNodePtr node,
    char *type);
...
static void get_type_str (
    ...
    const char *type,
    smart_str * ret);
static void set_ns_and_type_ex (
    xmlNodePtr node,
    char *ns,
    char *type);

static void set_ns_and_type (
    xmlNodePtr node,
    encodeTypePtr type);
...
encode defaultEncoding[] = {
    ...
    {{IS_STRING, XSD_STRING_STRING, XSD_NAMESPACE, NULL}, to_zval_string,
        to_xml_string},
    {{IS_LONG, XSD_INT_STRING, XSD_NAMESPACE, NULL}, to_zval_long,
        to_xml_long},
    {{IS_DOUBLE, XSD_FLOAT_STRING, XSD_NAMESPACE, NULL}, to_zval_double,
        to_xml_double},
    {{IS_BOOL, XSD_BOOLEAN_STRING, XSD_NAMESPACE, NULL}, to_zval_bool,
        to_xml_bool},
    {{IS_CONSTANT, XSD_STRING_STRING, XSD_NAMESPACE, NULL}, to_zval_string,
        to_xml_string},
    {{IS_ARRAY, SOAP_ENC_ARRAY_STRING, SOAP_1_1_ENC_NAMESPACE, NULL},
        to_zval_array, guess_array_map},
    {{IS_CONSTANT_ARRAY, SOAP_ENC_ARRAY_STRING, SOAP_1_1_ENC_NAMESPACE, NULL},
        to_zval_array, to_xml_array},
    {{IS_OBJECT, SOAP_ENC_OBJECT_STRING, SOAP_1_1_ENC_NAMESPACE, NULL},
        to_zval_object, to_xml_object},
    {{IS_ARRAY, SOAP_ENC_ARRAY_STRING, SOAP_1_2_ENC_NAMESPACE, NULL},
        to_zval_array, guess_array_map},
    {{IS_CONSTANT_ARRAY, SOAP_ENC_ARRAY_STRING, SOAP_1_2_ENC_NAMESPACE, NULL},
        to_zval_array, to_xml_array},
    {{IS_OBJECT, SOAP_ENC_OBJECT_STRING, SOAP_1_2_ENC_NAMESPACE, NULL},
        to_zval_object, to_xml_object},

    {{XSD_STRING, XSD_STRING_STRING, XSD_NAMESPACE, NULL}, to_zval_string,
        to_xml_string},
    {{XSD_BOOLEAN, XSD_BOOLEAN_STRING, XSD_NAMESPACE, NULL}, to_zval_bool,
        to_xml_bool},
    {{XSD_DECIMAL, XSD_DECIMAL_STRING, XSD_NAMESPACE, NULL}, to_zval_stringc,
        to_xml_string},
    ...
    {{XSD_GDAY, XSD_GDAY_STRING, XSD_NAMESPACE, NULL}, to_zval_stringc,
        to_xml_gday},
    ...
    {{XSD_INT, XSD_INT_STRING, XSD_NAMESPACE, NULL}, to_zval_long,
        to_xml_long},
    {{XSD_SHORT, XSD_SHORT_STRING, XSD_NAMESPACE, NULL}, to_zval_long,
        to_xml_long},
    {{XSD_BYTE, XSD_BYTE_STRING, XSD_NAMESPACE, NULL}, to_zval_long,
        to_xml_long},
    {{XSD_NONPOSITIVEINTEGER, XSD_NONPOSITIVEINTEGER_STRING, XSD_NAMESPACE,
                    NULL}, to_zval_long, to_xml_long},
    {{XSD_POSITIVEINTEGER, XSD_POSITIVEINTEGER_STRING, XSD_NAMESPACE, NULL},
        to_zval_long, to_xml_long},
    {{XSD_NONNEGATIVEINTEGER, XSD_NONNEGATIVEINTEGER_STRING, XSD_NAMESPACE,
                    NULL}, to_zval_long, to_xml_long},
    {{XSD_NEGATIVEINTEGER, XSD_NEGATIVEINTEGER_STRING, XSD_NAMESPACE, NULL},
        to_zval_long, to_xml_long},
    {{XSD_UNSIGNEDBYTE, XSD_UNSIGNEDBYTE_STRING, XSD_NAMESPACE, NULL},
        to_zval_long, to_xml_long},
    {{XSD_UNSIGNEDSHORT, XSD_UNSIGNEDSHORT_STRING, XSD_NAMESPACE, NULL},
        to_zval_long, to_xml_long},
    {{XSD_UNSIGNEDINT, XSD_UNSIGNEDINT_STRING, XSD_NAMESPACE, NULL},
        to_zval_long, to_xml_long},
    {{XSD_UNSIGNEDLONG, XSD_UNSIGNEDLONG_STRING, XSD_NAMESPACE, NULL},
        to_zval_long, to_xml_long},
    {{XSD_INTEGER, XSD_INTEGER_STRING, XSD_NAMESPACE, NULL}, to_zval_long,
        to_xml_long},

    {{XSD_ANYTYPE, XSD_ANYTYPE_STRING, XSD_NAMESPACE, NULL},
        guess_zval_convert, guess_xml_convert},
    {{XSD_UR_TYPE, XSD_UR_TYPE_STRING, XSD_NAMESPACE, NULL},
        guess_zval_convert, guess_xml_convert},
    {{XSD_ANYURI, XSD_ANYURI_STRING, XSD_NAMESPACE, NULL}, to_zval_stringc,
        to_xml_string},
    {{XSD_QNAME, XSD_QNAME_STRING, XSD_NAMESPACE, NULL}, to_zval_stringc,
        to_xml_string},
    {{XSD_NOTATION, XSD_NOTATION_STRING, XSD_NAMESPACE, NULL},
        to_zval_stringc, to_xml_string},
    {{XSD_NORMALIZEDSTRING, XSD_NORMALIZEDSTRING_STRING, XSD_NAMESPACE, NULL},
        to_zval_stringr, to_xml_string},
    {{XSD_TOKEN, XSD_TOKEN_STRING, XSD_NAMESPACE, NULL}, to_zval_stringc,
        to_xml_string},
    {{XSD_LANGUAGE, XSD_LANGUAGE_STRING, XSD_NAMESPACE, NULL},
        to_zval_stringc, to_xml_string},
    {{XSD_NMTOKEN, XSD_NMTOKEN_STRING, XSD_NAMESPACE, NULL}, to_zval_stringc,
        to_xml_string},
    {{XSD_NMTOKENS, XSD_NMTOKENS_STRING, XSD_NAMESPACE, NULL},
        to_zval_stringc, to_xml_list1},
    {{XSD_NAME, XSD_NAME_STRING, XSD_NAMESPACE, NULL}, to_zval_stringc,
        to_xml_string},
    {{XSD_NCNAME, XSD_NCNAME_STRING, XSD_NAMESPACE, NULL}, to_zval_stringc,
        to_xml_string},
    {{XSD_ID, XSD_ID_STRING, XSD_NAMESPACE, NULL}, to_zval_stringc,
        to_xml_string},
    {{XSD_IDREF, XSD_IDREF_STRING, XSD_NAMESPACE, NULL}, to_zval_stringc,
        to_xml_string},
    {{XSD_IDREFS, XSD_IDREFS_STRING, XSD_NAMESPACE, NULL}, to_zval_stringc,
        to_xml_list1},
    {{XSD_ENTITY, XSD_ENTITY_STRING, XSD_NAMESPACE, NULL}, to_zval_stringc,
        to_xml_string},
    {{XSD_ENTITIES, XSD_ENTITIES_STRING, XSD_NAMESPACE, NULL},
        to_zval_stringc, to_xml_list1},

    {{APACHE_MAP, APACHE_MAP_STRING, APACHE_NAMESPACE, NULL}, to_zval_map,
        to_xml_map},

    {{SOAP_ENC_OBJECT, SOAP_ENC_OBJECT_STRING, SOAP_1_1_ENC_NAMESPACE, NULL},
        to_zval_object, to_xml_object},
    {{SOAP_ENC_ARRAY, SOAP_ENC_ARRAY_STRING, SOAP_1_1_ENC_NAMESPACE, NULL},
        to_zval_array, to_xml_array},
    {{SOAP_ENC_OBJECT, SOAP_ENC_OBJECT_STRING, SOAP_1_2_ENC_NAMESPACE, NULL},
        to_zval_object, to_xml_object},
    {{SOAP_ENC_ARRAY, SOAP_ENC_ARRAY_STRING, SOAP_1_2_ENC_NAMESPACE, NULL},
        to_zval_array, to_xml_array},

    /* support some of the 1999 data types */
    {{XSD_STRING, XSD_STRING_STRING, XSD_1999_NAMESPACE, NULL},
        to_zval_string, to_xml_string},
    {{XSD_BOOLEAN, XSD_BOOLEAN_STRING, XSD_1999_NAMESPACE, NULL},
        to_zval_bool, to_xml_bool},
    {{XSD_DECIMAL, XSD_DECIMAL_STRING, XSD_1999_NAMESPACE, NULL},
        to_zval_stringc, to_xml_string},
    {{XSD_FLOAT, XSD_FLOAT_STRING, XSD_1999_NAMESPACE, NULL}, to_zval_double,
        to_xml_double},
    {{XSD_DOUBLE, XSD_DOUBLE_STRING, XSD_1999_NAMESPACE, NULL},
        to_zval_double, to_xml_double},

    {{XSD_LONG, XSD_LONG_STRING, XSD_1999_NAMESPACE, NULL}, to_zval_long,
        to_xml_long},
    {{XSD_INT, XSD_INT_STRING, XSD_1999_NAMESPACE, NULL}, to_zval_long,
        to_xml_long},
    {{XSD_SHORT, XSD_SHORT_STRING, XSD_1999_NAMESPACE, NULL}, to_zval_long,
        to_xml_long},
    ...
    {{XSD_1999_TIMEINSTANT, XSD_1999_TIMEINSTANT_STRING, XSD_1999_NAMESPACE,
                    NULL}, to_zval_stringc, to_xml_string},
    ...
    const char *type)
{
    if (sdl && sdl->encoders) {
        ...
        for (zend_hash_internal_pointer_reset_ex (sdl->encoders, &pos);
            zend_hash_get_current_data_ex (sdl->encoders, (void **) &enc,
                &pos) == SUCCESS;
    ...
    xmlNodePtr parent)
{
    ...
    if (data &&
        ...
        if (zend_hash_find (ht, "enc_type", sizeof ("enc_type"),
                (void **) & ztype) == FAILURE) {
            soap_error0 (E_ERROR,
                "Encoding: SoapVar hasn't 'enc_type' propery");
        }

        if (zend_hash_find (ht, "enc_stype", sizeof ("enc_stype"),
                (void **) & zstype) == SUCCESS) {
            if (zend_hash_find (ht, "enc_ns", sizeof ("enc_ns"),
                    (void **) & zns) == SUCCESS) {
                enc =
                    get_encoder (WSF_GLOBAL (sdl), Z_STRVAL_PP (zns),
                    Z_STRVAL_PP (zstype));
            } else {
                zns = NULL;
                enc =
                    get_encoder_ex (WSF_GLOBAL (sdl), Z_STRVAL_PP (zstype),
                    Z_STRLEN_PP (zstype));
            }
            if (enc == NULL && WSF_GLOBAL (typemap)) {
                ...
                if (zns != NULL) {
                    smart_str_appendl (&nscat, Z_STRVAL_PP (zns),
                        Z_STRLEN_PP (zns));
                    smart_str_appendc (&nscat, ':');
                }
                smart_str_appendl (&nscat, Z_STRVAL_PP (zstype),
                    Z_STRLEN_PP (zstype));
                smart_str_0 (&nscat);
                if (zend_hash_find (WSF_GLOBAL (typemap), nscat.c,
                        nscat.len + 1, (void **) &new_enc) == SUCCESS) {
                    enc = *new_enc;
                }
        ...
        if (zend_hash_find (ht, "enc_value", sizeof ("enc_value"),
                (void **) & zdata) == FAILURE) {
        ...
        if (zend_hash_find (ht, "enc_name", sizeof ("enc_name"),
                (void **) & zname) == SUCCESS) {
            xmlNodeSetName (node, BAD_CAST (Z_STRVAL_PP (zname)));
        }
        if (zend_hash_find (ht, "enc_namens", sizeof ("enc_namens"),
                (void **) & znamens) == SUCCESS) {
        ...
        if (WSF_GLOBAL (class_map) && data &&
            ...
            for (zend_hash_internal_pointer_reset_ex (WSF_GLOBAL (class_map),
                    &pos);
                zend_hash_get_current_data_ex (WSF_GLOBAL (class_map),
                    (void **) &tmp, &pos) == SUCCESS;
                zend_hash_move_forward_ex (WSF_GLOBAL (class_map), &pos)) {
                if (Z_TYPE_PP (tmp) == IS_STRING
                    && ce->name_length == Z_STRLEN_PP (tmp)
                    && zend_binary_strncasecmp (ce->name, ce->name_length,
                        Z_STRVAL_PP (tmp), ce->name_length,
                        ce->name_length) == 0
                    && zend_hash_get_current_key_ex (WSF_GLOBAL (class_map),
                        &type_name, &type_len, &idx, 0,
                        &pos) == HASH_KEY_IS_STRING) {

                    /* TODO: namespace isn't stored */
                    encodePtr enc =
                        get_encoder (WSF_GLOBAL (sdl),
                        WSF_GLOBAL (sdl)->target_ns, type_name);
                    if (enc) {
                        encode = enc;
                    } else if (WSF_GLOBAL (sdl)) {
                        enc =
                            find_encoder_by_type_name (WSF_GLOBAL (sdl),
                            type_name);
                        if (enc) {
                            encode = enc;
                        }
        ...
        if (WSF_GLOBAL (typemap) && encode->details.type_str) {
            ...
            smart_str_0 (&nscat);
            if (zend_hash_find (WSF_GLOBAL (typemap), nscat.c, nscat.len + 1,
                    (void **) &new_enc) == SUCCESS) {
                encode = *new_enc;
            }
            smart_str_free (&nscat);
        }
    ...
    xmlNodePtr data)
{
    ...
    if (WSF_GLOBAL (typemap)) {
        if (encode->details.type_str) {
            ...
            xmlAttrPtr type_attr =
                get_attribute_ex (data->properties, "type", XSI_NAMESPACE);
    ...
    xmlNodePtr data)
{
    ...
    if (encode == NULL) {
        ...
        if (type_attr != NULL) {
            encodePtr enc =
                get_encoder_from_prefix (WSF_GLOBAL (sdl), data,
    ...
    xmlNodePtr parent)
{
    ...
    if (type && type->map && type->map->to_xml) {
        ...
        if (call_user_function (EG (function_table), NULL, type->map->to_xml,
                return_value, 1, &data TSRMLS_CC) == FAILURE) {
            soap_error0 (E_ERROR, "Encoding: Error calling to_xml callback");
        }
        if (Z_TYPE_P (return_value) == IS_STRING) {
            xmlDocPtr doc =
                soap_xmlParseMemory (Z_STRVAL_P (return_value),
    ...
    xmlNodePtr node)
{
    ...
    if (type && type->map && type->map->to_zval) {
        ...
        if (call_user_function (EG (function_table), NULL, type->map->to_zval,
                return_value, 1, &data TSRMLS_CC) == FAILURE) {
            soap_error0 (E_ERROR,
    ...
    xmlNodePtr data)
{
    ...
    if (data && data->children) {
        if (data->children->type == XML_TEXT_NODE
            ...
            whiteSpace_collapse (data->children->content);
            str =
                (char *) php_base64_decode (data->children->content,
    ...
    xmlNodePtr parent)
{
    ...
    if (Z_TYPE_P (data) == IS_STRING) {
        str =
            php_base64_encode ((unsigned char *) Z_STRVAL_P (data),
        ...
        convert_to_string (&tmp);
        str =
            php_base64_encode ((unsigned char *) Z_STRVAL (tmp),
    ...
    xmlNodePtr parent)
{
    ...
    str =
        (unsigned char *) safe_emalloc (Z_STRLEN_P (data) * 2, sizeof (char),
        1);
    ...
    if (data && data->children) {
        if (data->children->type == XML_TEXT_NODE
            ...
            if (stricmp ((char *) data->children->content, "true") == 0 ||
                ...
                ZVAL_BOOL (ret, 1);
            } else if (stricmp ((char *) data->children->content,
                    "false") == 0
    ...
    char *name TSRMLS_DC)
{
    if (Z_TYPE_P (object) == IS_OBJECT) {
        ...
        EG (scope) = Z_OBJCE_P (object);
        data =
            Z_OBJ_HT_P (object)->read_property (object, &member,
            BP_VAR_IS TSRMLS_CC);
        if (data == EG (uninitialized_zval_ptr)) {
            ...
            property_info =
                zend_get_property_info (Z_OBJCE_P (object), &member,
                1 TSRMLS_CC);
            EG (scope) = old_scope;
            if (property_info
                && zend_hash_quick_exists (Z_OBJPROP_P (object),
    ...
    while (node != NULL) {
        if (get_zval_property (ret, (char *) node->name TSRMLS_CC) == NULL) {
            ...
            if (get_attribute_ex (node->properties, "type",
                    XSI_NAMESPACE) == NULL && Z_TYPE_P (val) == IS_STRING) {
                while (node->next != NULL
                    && get_zval_property (ret,
                        (char *) node->next->name TSRMLS_CC) == NULL
                    && get_attribute_ex (node->next->properties, "type",
                        XSI_NAMESPACE) == NULL) {
                    zval *val2 =
                        master_to_zval (get_conversion (XSD_ANYXML),
                        node->next);
    ...
    sdlPtr sdl TSRMLS_DC)
{
    ...
    case XSD_CONTENT_ELEMENT:
        if (model->u.element->name) {
            xmlNodePtr node =
                get_node (data->children, model->u.element->name);
            ...
            if (node) {
                ...
                if (node && node->children && node->children->content) {
                    if (model->u.element->fixed
                        && strcmp (model->u.element->fixed,
                            (char *) node->children->content) != 0) {
                        soap_error3 (E_ERROR,
                            "Encoding: Element '%s' has fixed value '%s' (value '%s' is not allowed)",
                            model->u.element->name, model->u.element->fixed,
                    ...
                    xmlFreeNode (dummy);
                } else if (model->u.element->def
                    && !model->u.element->nillable) {
                    xmlNodePtr dummy = xmlNewNode (NULL, BAD_CAST ("BOGUS"));
                    xmlNodeSetContent (dummy,
                        BAD_CAST (model->u.element->def));
                    val = master_to_zval (model->u.element->encode, dummy);
                    xmlFreeNode (dummy);
                } else {
                    val = master_to_zval (model->u.element->encode, node);
                ...
                if ((node =
                    ...
                    do {
                        if (node && node->children && node->children->content) {
                            ...
                            val =
                                master_to_zval (model->u.element->encode,
                                node);
                        } else if (model->u.element->fixed) {
                            xmlNodePtr dummy =
                ...
                set_zval_property (ret, model->u.element->name,
                    val TSRMLS_CC);
            }
        }
        break;
    ...
    case XSD_CONTENT_GROUP:
        model_to_zval_object (ret, model->u.group->model, data,
            sdl TSRMLS_CC);
        break;
    default:
        break;
    ...
    zend_class_entry * pce)
{
    ...
    if (pce) {
        ...
        if (zend_hash_find (WSF_GLOBAL (class_map), type->type_str,
                strlen (type->type_str) + 1, (void **) &classname) == SUCCESS
            && Z_TYPE_PP (classname) == IS_STRING
            && (tmp =
                zend_fetch_class (Z_STRVAL_PP (classname),
                    Z_STRLEN_PP (classname),
    ...
    if (sdlType) {
                ...
                if (ce != ZEND_STANDARD_CLASS_DEF_PTR &&
                    ...
                    && (sdlType->encode->details.sdl_type->encode == NULL
                        || (sdlType->encode->details.sdl_type->encode->
                            details.type != IS_ARRAY
                            && sdlType->encode->details.sdl_type->encode->
                            details.type != SOAP_ENC_ARRAY))) {
                    ret =
                        to_zval_object_ex (&sdlType->encode->details, data,
                        ce);
                } else {
            ...
            while (zend_hash_get_current_data_ex (sdlType->attributes,
                ...
                if ((*attr)->name) {
                    ...
                    if (str_val) {
                        ...
                        xmlFreeNode (dummy);
                        set_zval_property (ret, (*attr)->name,
                            data TSRMLS_CC);
                    }
                }
        ...
        while (trav != NULL) {
            if (trav->type == XML_ELEMENT_NODE) {
                ...
                if (!prop) {
                    set_zval_property (ret, (char *) trav->name,
                        tmpVal TSRMLS_CC);
                } else {
    ...
    int strict TSRMLS_DC)
{
    ...
    case XSD_CONTENT_ELEMENT:{
            ...
            data =
                get_zval_property (object, model->u.element->name TSRMLS_CC);
            if (data) {
                enc = model->u.element->encode;
                if ((model->max_occurs == -1 || model->max_occurs > 1) &&
                    Z_TYPE_P (data) == IS_ARRAY && !is_map (data)) {
                    HashTable *ht = Z_ARRVAL_P (data);
                    zval **val;

                    zend_hash_internal_pointer_reset (ht);
                    while (zend_hash_get_current_data (ht,
                            (void **) &val) == SUCCESS) {
                    ...
                    if (Z_TYPE_P (data) == IS_NULL
                        ...
                        if (property->children && property->children->content
                            && model->u.element->fixed
                            && strcmp (model->u.element->fixed,
                                (char *) property->children->content) != 0) {
                            soap_error3 (E_ERROR,
                                "Encoding: Element '%s' has fixed value '%s' (value '%s' is not allowed)",
                                model->u.element->name,
                                model->u.element->fixed,
                ...
                && model->min_occurs > 0) {
                property =
                    xmlNewNode (NULL, BAD_CAST (model->u.element->name));
                xmlAddChild (node, property);
                set_xsi_nil (property);
                ...
                if (strict) {
                    soap_error1 (E_ERROR,
                        "Encoding: object hasn't '%s' property",
                        model->u.element->name);
                }
                return 0;
            }
    ...
    case XSD_CONTENT_ANY:{
            ...
            if (data) {
                ...
                if (strict) {
                    soap_error0 (E_ERROR,
                        "Encoding: object hasn't 'any' property");
                }
                return 0;
            }
            break;
    ...
    case XSD_CONTENT_ALL:{
            ...
            while (zend_hash_get_current_data_ex (model->u.content,
                    (void **) &tmp, &pos) == SUCCESS) {
                if (!model_to_xml_object (node, *tmp, object, style,
                        model->min_occurs > 0 TSRMLS_CC)) {
                    return 0;
                }
                zend_hash_move_forward_ex (model->u.content, &pos);
            }
    ...
    case XSD_CONTENT_CHOICE:{
            ...
            while (zend_hash_get_current_data_ex (model->u.content,
                    (void **) &tmp, &pos) == SUCCESS) {
                int tmp_ret =
                    model_to_xml_object (node, *tmp, object, style,
                    0 TSRMLS_CC);
                if (tmp_ret == 1) {
                    return 1;
                } else if (tmp_ret != 0) {
    ...
    case XSD_CONTENT_GROUP:{
            return model_to_xml_object (node, model->u.group->model, object,
    ...
    if (sdlType) {
        ...
        if (prop != NULL) {
            ...
            if (Z_TYPE_P (data) == IS_ARRAY &&
                ...
                while (zend_hash_get_current_data (prop,
                        (void **) &val) == SUCCESS) {
                    xmlNodePtr property;
                    if (Z_TYPE_PP (val) == IS_NULL && array_el->nillable) {
                        ...
                        property =
                            master_to_xml (array_el->encode, *val, style,
                            xmlParam);
                    }
                ...
                model_to_xml_object (xmlParam, sdlType->model, data, style,
                    1 TSRMLS_CC);
            }
            if (sdlType->attributes) {
                sdlAttributePtr *attr;
                ...
                zend_hash_internal_pointer_reset_ex (sdlType->attributes,
                    &pos);
                while (zend_hash_get_current_data_ex (sdlType->attributes,
                        (void **) &attr, &pos) == SUCCESS) {
                    if ((*attr)->name) {
                        zattr =
                            get_zval_property (data, (*attr)->name TSRMLS_CC);
                        if (zattr) {
                            xmlNodePtr dummy;

                            dummy =
                                master_to_xml ((*attr)->encode, zattr,
                            ...
                            if (dummy->children && dummy->children->content) {
                                 ...
                                 */
                                if ((*attr)->namens &&
                                    (!strncmp ((*attr)->namens, XML_NAMESPACE,
                                            sizeof (XML_NAMESPACE))
                                        || (*attr)->form ==
                                        XSD_FORM_QUALIFIED)) {
                                    xmlNsPtr nsp =
                                        encode_add_ns (xmlParam,
                                        (*attr)->namens);

                                    xmlSetNsProp (xmlParam, nsp,
                                        BAD_CAST ((*attr)->name),
                                        dummy->children->content);
                                } else {
                                    xmlSetProp (xmlParam,
        ...
        if (prop != NULL) {
            ...
            for (; i > 0; i--) {
                ...
                key_type =
                    zend_hash_get_current_key_ex (prop, &str_key,
                    &str_key_len, &index, FALSE, NULL);
                zend_hash_get_current_data (prop, (void **) &zprop);

                property =
                    master_to_xml (get_conversion ((*zprop)->type), (*zprop),
                    style, xmlParam);
...
#else
                        zend_unmangle_property_name (str_key, str_key_len - 1,
    ...
    int style)
{
    ...
    if (data && Z_TYPE_P (data) == IS_ARRAY) {
        ...
        for (j = 0; j < dims[0]; j++) {
            ...
            if (zend_hash_get_current_data (data->value.ht,
                    (void **) &zdata) != SUCCESS) {
                zdata = NULL;
            }
            if (dimension == 1) {
                ...
                if (zdata) {
                    if (enc == NULL) {
                        xparam =
                            master_to_xml (get_conversion ((*zdata)->type),
                            (*zdata), style, xmlParam);
                    } else {
                        xparam =
                            master_to_xml (enc, (*zdata), style, xmlParam);
                    }
                } else {
                ...
                if (zdata) {
                    add_xml_array_elements (xmlParam, type, enc, ns,
                        dimension - 1, dims + 1, *zdata, style);
                } else {
                    add_xml_array_elements (xmlParam, type, enc, ns,
                        dimension - 1, dims + 1, NULL, style);
                }
            }
    ...
    xmlNodePtr parent)
{
    ...
    smart_str array_type = { 0 }, array_size = {
    0};
    int i;
    xmlNodePtr xmlParam;
    encodePtr enc = NULL;
    ...
    if (Z_TYPE_P (data) == IS_ARRAY) {
        ...
        if (sdl_type &&
            ...
            && zend_hash_find ((*arrayType)->extraAttributes,
                ...
                sizeof (WSDL_NAMESPACE ":arrayType"),
                (void **) &ext) == SUCCESS) {
            ...
            if ((*ext)->ns != NULL) {
                enc = get_encoder (WSF_GLOBAL (sdl), (*ext)->ns, value);
            ...
            && zend_hash_find ((*arrayType)->extraAttributes,
                ...
                sizeof (WSDL_NAMESPACE ":itemType"),
                (void **) &ext) == SUCCESS) {
            if ((*ext)->ns != NULL) {
                enc = get_encoder (WSF_GLOBAL (sdl), (*ext)->ns, (*ext)->val);
        ...
        if (style == SOAP_ENCODED) {
            if (soap_version == SOAP_1_1) {
                ...
                if (strcmp (array_type.c, "xsd:anyType") == 0) {
                    ...
                    smart_str_appendl (&array_type, "xsd:ur-type",
                        sizeof ("xsd:ur-type") - 1);
                }
        ...
        add_xml_array_elements (xmlParam, element_type, enc,
            enc ? encode_add_ns (xmlParam, enc->details.ns) : NULL, dimension,
            dims, data, style);
        efree (dims);
    }
    if (style == SOAP_ENCODED) {
    ...
    xmlNodePtr data)
{
    ...
    if (data &&
        ...
        if (nsptr != NULL) {
            enc = get_encoder (WSF_GLOBAL (sdl), (char *) nsptr->href, type);
        }
        efree (type);
        if (ns) {
            efree (ns);
        ...
        if ((attr = get_attribute (data->properties, "arraySize")) &&
            ...
            dims =
                get_position_12 (dimension, (char *) attr->children->content);
        } else {
            dims = emalloc (sizeof (int));
            *dims = 0;
        }
        ...
        zend_hash_find (type->sdl_type->attributes,
            ...
            (void **) &arrayType) == SUCCESS
        && zend_hash_find ((*arrayType)->extraAttributes,
            WSDL_NAMESPACE ":arrayType", sizeof (WSDL_NAMESPACE ":arrayType"),
            (void **) &ext) == SUCCESS) {
        char *type, *end;
        ...
        if ((*ext)->ns != NULL) {
            enc = get_encoder (WSF_GLOBAL (sdl), (*ext)->ns, type);
        }
        efree (type);
        ...
        *dims = 0;
        ...
        zend_hash_find (type->sdl_type->attributes,
            ...
            (void **) &arrayType) == SUCCESS
        && zend_hash_find ((*arrayType)->extraAttributes,
            WSDL_NAMESPACE ":itemType", sizeof (WSDL_NAMESPACE ":itemType"),
            (void **) &ext) == SUCCESS) {
        ...
        zend_hash_find (type->sdl_type->attributes,
            ...
            (void **) &arrayType) == SUCCESS
        && zend_hash_find ((*arrayType)->extraAttributes,
            WSDL_NAMESPACE ":arraySize", sizeof (WSDL_NAMESPACE ":arraysize"),
            (void **) &ext) == SUCCESS) {
    ...
    while (trav) {
        if (trav->type == XML_ELEMENT_NODE) {
            ...
            while (i < dimension - 1) {
                zval **ar2;
                if (zend_hash_index_find (Z_ARRVAL_P (ar), pos[i],
                        (void **) &ar2) == SUCCESS) {
                    ar = *ar2;
                } else {
                    zval *tmpAr;
                    MAKE_STD_ZVAL (tmpAr);
                    array_init (tmpAr);
                    zend_hash_index_update (Z_ARRVAL_P (ar), pos[i], &tmpAr,
                        sizeof (zval *), (void **) &ar2);
                    ar = *ar2;
                }
                i++;
            }
    ...
    if (Z_TYPE_P (data) == IS_ARRAY) {
        ...
        for (; i > 0; i--) {
            ...
            if (Z_TYPE_PP (temp_data) != IS_NULL) {
                ...
                xparam =
                    master_to_xml (get_conversion ((*temp_data)->type),
    ...
    if (data && data->children) {
        ...
        FOREACHNODE (trav, "item", item) {
            ...
            if (Z_TYPE_P (key) == IS_STRING) {
                zend_hash_update (Z_ARRVAL_P (ret), Z_STRVAL_P (key),
                    Z_STRLEN_P (key) + 1, &value, sizeof (zval *), NULL);
            } else if (Z_TYPE_P (key) == IS_LONG) {
                zend_hash_index_update (Z_ARRVAL_P (ret), Z_LVAL_P (key),
...
/*
	if (style == SOAP_LITERAL && WSF_GLOBAL(sdl)) {
    ...
    xmlNodePtr data)
{
    ...
    if (WSF_GLOBAL (sdl) && type_name && enc->details.sdl_type) {
        zval *soapvar;
        char *ns, *cptype;
        xmlNsPtr nsptr;

        MAKE_STD_ZVAL (soapvar);
        object_init_ex (soapvar, ws_var_class_entry);
...
# ifdef __CYGWIN__
        sprintf (tzbuf, "%c%02d:%02d",
            ((ta->tm_isdst ? _timezone - 3600 : _timezone) > 0) ? '-' : '+',
            abs ((ta->tm_isdst ? _timezone - 3600 : _timezone) / 3600),
            abs (((ta->tm_isdst ? _timezone -
                        3600 : _timezone) % 3600) / 60));
# else
        sprintf (tzbuf, "%c%02d:%02d",
            ((ta->tm_isdst ? timezone - 3600 : timezone) > 0) ? '-' : '+',
            abs ((ta->tm_isdst ? timezone - 3600 : timezone) / 3600),
            abs (((ta->tm_isdst ? timezone - 3600 : timezone) % 3600) / 60));
# endif
#endif
...
to_zval_list (
    ...
    xmlNodePtr data)
{
     /*FIXME*/ return to_zval_stringc (enc, data);
}
    ...
    xmlNodePtr parent)
{
    ...
    if (enc->sdl_type && enc->sdl_type->kind == XSD_TYPEKIND_LIST
        && enc->sdl_type->elements) {
        sdlTypePtr *type;

        zend_hash_internal_pointer_reset (enc->sdl_type->elements);
        if (zend_hash_get_current_data (enc->sdl_type->elements,
                (void **) &type) == SUCCESS) {
    ...
    if (Z_TYPE_P (data) == IS_ARRAY) {
        ...
        while (zend_hash_get_current_data (ht, (void **) &tmp) == SUCCESS) {
            xmlNodePtr dummy =
                master_to_xml (list_enc, *tmp, SOAP_LITERAL, ret);
...
to_zval_union (
    ...
    xmlNodePtr data)
{
     /*FIXME*/ return to_zval_list (enc, data);
}
...
to_xml_union (
    ...
    xmlNodePtr parent)
{
     /*FIXME*/ return to_xml_list (enc, data, style, parent);
}
    ...
    if (data && data->properties) {
        ...
        if (href) {
            ...
            if (href->children->content[0] == '#') {
                xmlNodePtr ret =
                    get_node_with_attribute_recursive (data->doc->children,
                    NULL, "id", (char *) &href->children->content[1]);
                if (!ret) {
                    soap_error1 (E_ERROR,
                        "Encoding: Unresolved reference '%s'",
                        href->children->content);
                }
                return ret;
            } else {
                /*  TODO: External href....? */
                soap_error1 (E_ERROR, "Encoding: External reference '%s'",
                    href->children->content);
            }
        }
        /* SOAP 1.2 enc:id enc:ref */
        href =
            get_attribute_ex (data->properties, "ref",
        ...
        if (href) {
            ...
            ret =
                get_node_with_attribute_recursive_ex (data->doc->children,
                NULL, NULL, "id", (char *) id, SOAP_1_2_ENC_NAMESPACE);
            if (!ret) {
                soap_error1 (E_ERROR, "Encoding: Unresolved reference '%s'",
                    href->children->content);
            } else if (ret == data) {
                soap_error1 (E_ERROR,
                    "Encoding: Violation of id and ref information items '%s'",
                    href->children->content);
            }
            return ret;
        }
    ...
    const char *ns)
{
    ...
    if (xmlns == NULL) {
        ...
        if (zend_hash_find (&WSF_GLOBAL (defEncNs), (char *) ns,
                strlen (ns) + 1, (void **) &prefix) == SUCCESS) {
            ...
            while (1) {
                ...
                prefix.c = NULL;
                prefix.len = 0;
                num = ++WSF_GLOBAL (cur_uniq_ns);
            }

            xmlns =
                xmlNewNs (node->doc->children, BAD_CAST (ns),
                BAD_CAST (prefix.c));
            smart_str_free (&prefix);
        }
    }
...
set_ns_prop (
    ...
    char *val)
{
    xmlSetNsProp (node, encode_add_ns (node, ns), BAD_CAST (name),
        BAD_CAST (val));
}
    ...
    int encode)
{
    ...
    if (zend_hash_index_find (&WSF_GLOBAL (defEncIndex), encode,
            (void **) &enc) == FAILURE) {
    ...
    smart_str * type TSRMLS_DC)
{
        ...
        if (i > 0) {
            if ((cur_type != prev_type) ||
                ...
                || (cur_ns != NULL && prev_ns != NULL
                    && strcmp (cur_ns, prev_ns) != 0) || (cur_ns == NULL
                    && cur_ns != prev_ns)) {
                different = TRUE;
                break;
            }
    ...
    if (different || count == 0) {
        ...
        if (cur_stype != NULL) {
            ...
            enc =
                get_encoder_ex (WSF_GLOBAL (sdl), array_type.c,
                array_type.len);
            smart_str_free (&array_type);
            return enc;
        } else {
    ...
    smart_str * ret)
{
    ...
    if (ns) {
        ...
        } else if (WSF_GLOBAL (soap_version) == SOAP_1_1 &&
