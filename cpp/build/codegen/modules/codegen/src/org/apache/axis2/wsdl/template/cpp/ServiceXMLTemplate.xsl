...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="interface">
        ...
        <service>
            ...
            <parameter>
                <xsl:attribute name='name'>ServiceClass</xsl:attribute>
                <xsl:value-of select="$servicename"/>
            </parameter>
            <description>
              <xsl:value-of select="@servicename"/>Service</description>
			<xsl:for-each select="method">
				<operation>
                    ...
                    <messageReceiver>
                        <xsl:attribute name="class">wsf_cpp_msg_recv</xsl:attribute>
                    </messageReceiver>
