...
public class WSASModuleResultPage extends AbstractModuleValidateWizardPage{
	
	Browser browser;
	Button browserPopUpCheck;
	
    public WSASModuleResultPage(){
        super("page2");
    }
    ...
    public void createControl(Composite parent) {
		...
		browserPopUpCheck.setText(WSASModuleValidatePlugin.getResourceString("page2.browser.check"));
	...
	public void fillBrowserWithResults(){
		if(WSASModuleValidatePlugin.getDefault().isGoAheadValidation()){
				URL url = WSASModuleValidatePlugin.getDefault().getValidateURL();
				if(browser.setUrl(url.toString())){
					updateStatus(null);
				}else{
					updateStatus("Error Occured Displaying Results !!");
				}
				WSASModuleValidatePlugin.getDefault().setGoAheadValidation(false);
		}
	}
