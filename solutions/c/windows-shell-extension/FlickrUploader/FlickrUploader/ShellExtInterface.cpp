...
{
	hbmp_WS = LoadBitmap (((CComModule*)&_AtlModule)->GetModuleInstance(), MAKEINTRESOURCE(IDB_BITMAP_CON_MENU_WS));
	hbmp_Flickr = LoadBitmap (((CComModule*)&_AtlModule)->GetModuleInstance(), MAKEINTRESOURCE(IDB_BITMAP_CON_MENU_FLICKR));
}
...
STDMETHODIMP CShellExtInterface::InvokeCommand (LPCMINVOKECOMMANDINFO pCmdInfo)
{
	...
	switch (LOWORD(pCmdInfo->lpVerb))
	{
		case 0:
			{
				CFileUploaderDialog* pDialog = new CFileUploaderDialog;
				pDialog->SetFileNames (lst_FileName);
				pDialog->DoModal ();
				//pDialog->Create (IDD_DIALOG_FILE_UPLOADER);
