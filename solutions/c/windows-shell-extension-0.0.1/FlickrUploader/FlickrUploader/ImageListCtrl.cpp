...
void CImageListCtrl::OnLButtonDown (UINT nFlags, CPoint point)
{
	...
	if (iItemIndex != -1) // Item Selected at the Moment
	{
...
void CImageListCtrl::OnNMClick (NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (iItemIndex != -1) // Item Selected
	{
		...
		((CFileUploaderDialog*)GetParent ())->EnableUploadButton (TRUE);
	}
	else
	{
		((CFileUploaderDialog*)GetParent ())->EnableUploadButton (FALSE);
	}
