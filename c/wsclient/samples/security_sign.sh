...
then
    ...
    if [ `uname -s` = Darwin ]
    then
        sed -e 's,AXIS2C_HOME,'$INST_DIR',g' -e 's,\.so,\.dylib,g' data/security_sign_services.xml > $SERVICE_HOME/services.xml
    else
        sed 's,AXIS2C_HOME,'$INST_DIR',g' data/security_sign_services.xml > $SERVICE_HOME/services.xml
    fi
