...
import org.wso2.wsf.codegen.util.Constants;
...
public class OptionsPage extends AbstractWizardPage implements UIConstants {
	...
	private HashMap<String, String[]> getInitialisedAdvanceOptions(){
		HashMap<String, String[]> advanceOptions=new HashMap<String, String[]>();
	...
	public void createControl(Composite parent) {
	    ...
	    label.setText(Activator.getResourceString("page1.options.desc"));
		...
		label1.setText(Activator
				.getResourceString("page1.language.caption"));
		...
		languageSelectionComboBox.addSelectionListener(new SelectionListener() {
			public void widgetSelected(SelectionEvent e) {
				settings.put(PREF_LANGUAGE_INDEX, languageSelectionComboBox
						.getSelectionIndex());
				System.out.println(languageSelectionComboBox.getSelectionIndex());
				if(languageSelectionComboBox.getSelectionIndex() <= 1){
					disableInvalidControlsForCAndCPP();
					/** disable outpage java specific options */
					disableOutpageControls();
				}else{
			        testCaseCheckBoxButton.setEnabled(true);
			        databindingTypeCombo.setEnabled(true);
			        languageSelectionComboBox.setEnabled(true);
			        namespace2packageTable.setEnabled(true);
			        packageText.setEditable(true);
			        portNameCombo.setEnabled(true);
			        if(!clientSideCheckBoxButton.isEnabled() && !serverSideCheckBoxButton.isEnabled()){
			        	clientSideCheckBoxButton.setEnabled(true);
			        	syncOnlyRadioButton.setEnabled(true);
				        asyncOnlyRadioButton.setEnabled(true);
				        syncAndAsyncRadioButton.setEnabled(true);
			        }
			        serviceNameCombo.setEnabled(true);
			        packageText.setEnabled(true);
			        generateAllCheckBoxButton.setEnabled(true);
			        advanceOptionsButton.setEnabled(true);
			        /** Enable the output page java sepecific options */
			        enableOutpageControls();
				}
		...
		label.setText(Activator
				.getResourceString("page1.serviceName.caption"));
		...
		label = new Label(container, SWT.NULL);
		label.setText(Activator
				.getResourceString("page1.portName.caption"));
		...
		label = new Label(container, SWT.NULL);
		label.setText(Activator
				.getResourceString("page1.databindingCheck.caption"));
		...
		label = new Label(container, SWT.NULL);
		label.setText(Activator
				.getResourceString("page1.package.caption"));
		...
		testCaseCheckBoxButton
				.setText(Activator
						.getResourceString("page1.testcase.caption"));
		...
		clientSideCheckBoxButton.setText(Activator
				.getResourceString("page1.clientside.caption"));
		...
		syncAndAsyncRadioButton.setText(Activator
				.getResourceString("page1.syncAsync.caption"));
		...
		syncOnlyRadioButton.setText(Activator
				.getResourceString("page1.sync.caption"));
		...
		asyncOnlyRadioButton
				.setText(Activator
						.getResourceString("page1.async.caption"));
		...
		serverSideCheckBoxButton.setText(Activator
				.getResourceString("page1.serverside.caption"));
		...
		serverXMLCheckBoxButton.setText(Activator
				.getResourceString("page1.serviceXML.caption"));
		...
		generateServerSideInterfaceCheckBoxButton.setText(Activator
				.getResourceString("page1.ssInterface.caption"));
		...
		generateAllCheckBoxButton.setText(Activator
				.getResourceString("page1.genAll.caption"));
		...
		label = new Label(container, SWT.NULL);
		label.setText(Activator
				.getResourceString("page1.namespace2Pkg.caption"));
        ...
        declareColumn(namespace2packageTable,
        		...
        		Activator
				.getResourceString("page1.namespace.caption"));
        declareColumn(namespace2packageTable,
	...
	private void handleAdvanceButtonClick(){
		WSDLJavaAdvanceDialog javaAdvanceDialog = new WSDLJavaAdvanceDialog(getShell(),
				advanceOptions, languageSelectionComboBox.getSelectionIndex() == 2 ? true :false);
	...
	public void populateParamsFromWSDL() {
		...
		try {
			...
			updateStatus(Activator
					.getResourceString("page1.wsdlInvalid.message"));
		}
		catch (Exception e) {
			// disable the combo's
			setComboBoxEnable(false);
			updateStatus(Activator
					.getResourceString("page1.wsdlNotFound.message"));
		}
	...
	private void loadPortNames() {
		...
		if (selectionIndex != -1) {
			...
			if (!ports.isEmpty()) {
				...
				updateStatus(Activator
						.getResourceString("page1.noports.message"));
			}
		}
	}
	...
	private void fillLanguageCombo() {
		languageSelectionComboBox.add(CPP);
		languageSelectionComboBox.add(C);
	...
	private void handleServersideSelection() {
		if (this.serverSideCheckBoxButton.getSelection()) {
			...
			if(languageSelectionComboBox.getSelectionIndex() == 2){
	...
	private void disableInvalidControlsForCAndCPP(){
		this.testCaseCheckBoxButton.setEnabled(false);
        this.testCaseCheckBoxButton.setSelection(false);
        this.generateServerSideInterfaceCheckBoxButton.setEnabled(false);
        this.generateServerSideInterfaceCheckBoxButton.setSelection(false);
	...
	private void enableOutpageControls(){
		((OutputPage)getCodegenWizard().getOutPage()).enableLangDependentControls();
	}
	private void disableOutpageControls(){
		((OutputPage)getCodegenWizard().getOutPage()).disableLangDependentControls();
	}
